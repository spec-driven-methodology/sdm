...
When `--strict` is set, kit export SHALL fail with `KIT_EXPORT_BLOCKED` if critical kit-readiness warnings exist (`KIT_NO_PROBE_QUESTION`, `KIT_SKILL_DESCRIPTION_EMPTY`, or `KIT_EXPLANATION_MISSING`).
