...
- `name`: logical id `sdm`
- `title`: `Specra`
- `description`: human string that includes `Spec-based Resource Assessment Framework` and the full product identity from SSOT (e.g. with a `v` prefix), including stage and build when present
...
- **THEN** it contains `Spec-based Resource Assessment Framework` and the same identity string as `serverInfo.version`
