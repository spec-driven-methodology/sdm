...
Реестр MCP-хостов и установка stdio SDM MCP в конфиги агентов (`cursor`, `gigacode`, …) без ручного редактирования путей.
...
The system SHALL merge SDM stdio MCP server config (`command`, `args`, optional `env.SDM_PROJECT_ROOT`) into each selected host via `sdm mcp install --hosts <csv>|all`. Flag `--cursor` SHALL be treated as selecting host `cursor`. Non-interactive runs without `--hosts` and without `--cursor` SHALL fail with `HOSTS_REQUIRED` (unless a future TTY selection succeeds).

The default MCP servers map key SHALL be `SDM` (capital S) so host UIs that display the config key (e.g. Cursor sidebar) show the product brand. Callers MAY override the key with `--name` / `serverName`. The MCP protocol implementation `name` field remains the logical id `sdm` and is independent of the host config key.

When installing with the default key `SDM`, if a legacy key `sdm` already exists for a SDM MCP entry, the install SHALL write under `SDM` and SHALL remove the legacy `sdm` key to avoid duplicate servers.
...
- **THEN** both host configs are written under key `SDM` and JSON lists each resulting path
...
- **THEN** the file contains `mcpServers.SDM` and MUST NOT still contain `mcpServers.sdm`

#### Scenario: Custom --name is preserved

- **WHEN** agent runs `sdm mcp install --hosts cursor --cursor-root <dir> --name my-sdm --json`
- **THEN** `<dir>/.cursor/mcp.json` contains `mcpServers["my-sdm"]` (not necessarily `SDM`)

### Requirement: MCP install wires portable skills by default

Successful `sdm mcp install --hosts …` SHALL also install portable SDM skills into the same selected hosts (same host flags / roots as `sdm agent install`), unless the caller passes `--no-skills`. With `--json`, the payload SHALL include a `skills` object describing the skills install result when skills ran, or `skills: null` when `--no-skills` was set. If MCP config write succeeds but skills install fails, the command SHALL fail with a stable SdmError code (non-zero exit) and MUST still report which MCP paths were written when `--json` is used.
...
- **THEN** `<dir>/.cursor/mcp.json` contains `mcpServers.SDM`
...
When `mcp install` runs without `--project`, the written SDM server entry SHALL omit `env.SDM_PROJECT_ROOT` even if a previous entry on that host had that env set (full replace of the SDM server object for the chosen key).

#### Scenario: Stale SDM_PROJECT_ROOT removed

- **WHEN** `<dir>/.cursor/mcp.json` has `mcpServers.SDM.env.SDM_PROJECT_ROOT` set to a methodology path
- **AND** agent runs `sdm mcp install --hosts cursor --cursor-root <dir> --json` without `--project`
- **THEN** the SDM server entry has no `SDM_PROJECT_ROOT` env

### Requirement: Connect-mcp documents SDM install key

Portable skill `agents/connect-mcp/` SHALL document that the default host config key is `SDM` (Cursor/GigaCode sidebar label) and that hosts often show the config key rather than MCP `title`. It SHALL tell agents to re-run `mcp install` after upgrades when the key or entry path changes.

#### Scenario: Skill mentions SDM key and sidebar behavior

- **WHEN** an agent reads `agents/connect-mcp/SKILL.md`
- **THEN** the skill mentions default key `SDM` and that Cursor may display the `mcp.json` key
