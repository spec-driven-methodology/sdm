...
import { addQuestion, addSkill, appendActionLog, buildAbout, buildSuggest, createCertification, createProfile, patchCertification, reweightCertification, exportDocumentPayload, exportCourse, exportMatrix, exportMermaid, exportTest, exportConfluence, exportKit, findProjectRoot, formatAboutText, formatSuggestText, getProductVersion, initMethodologyProject, isMethodologyProject, hasLegacyRolesDirectory, syncPlayerAssets, syncStudioAssets, pushStudioView, pushStudioCoverage, pullStudioAction, startStudioServe, linkSkill, listQuestions, listTerms, addTerm, generateQuestions, validateQuestion, validateQuestionLibraryDeep, inferEdgesForProject, healCourseWarnings, parseIntentPlanJson, parseSkillIdList, runCertCoverage, runCertGaps, runSkillGraph, runSkillImpact, runContentStale, runMethodologyAudit, buildQualityReport, formatQualityReportText, rebuildSemanticIndex, searchSemanticIndex, buildTopicRegistry, syncTopicRegistryFromSkills, addTopic, mergeRegistryLabelsIntoSkills, SdmError, } from "@spec-driven-methodology/core";
import { installAgentSkills, listAgentHosts, } from "./agent-hosts.js";
import { registerCompletion } from "./completion.js";
import { helpBannerText } from "./banner.js";
import { buildMcpConfig, DEFAULT_MCP_SERVER_NAME, formatSdmVersionOutput, } from "./mcp-config.js";
import { hostConfigPath, installMcpHosts, listHosts, resolveHostsFromCli, } from "./mcp-hosts.js";
function parseRubricRows(raw) {
    return raw.map((line) => {
        const idx = line.indexOf(":");
        if (idx <= 0) {
            throw new SdmError("VALIDATION_FAILED", `Invalid --rubric "${line}". Expected score:description (score 0-3)`);
        }
        ...
        if (![0, 1, 2, 3].includes(score) || !description) {
            throw new SdmError("VALIDATION_FAILED", `Invalid --rubric "${line}". Score must be 0-3 with non-empty description`);
        }
...
function startDirFromOpts(opts) {
...
let lastCliErrorCode;
function emitError(err, asJson) {
...
program
    ...
    .option("--with-examples", "Seed example ontology, questions, and java-developer/middle certification")
    .option("-f, --force", "Overwrite existing files", false)
    .option("-d, --dir <path>", "Parent directory for the project (default: cwd)", process.cwd())
    .option("--subdir <name>", "Create a new subdirectory with this name and init inside it")
    .action((opts) => {
    const targetDir = resolve(opts.dir);
    if (isMethodologyProject(targetDir) && !opts.force) {
        console.error(`Already a SDM project (${targetDir}/sdm.yaml). Use --force to re-init.`);
        process.exitCode = 1;
        return;
    }
    ...
    console.log("  # Cursor: sdm mcp install --hosts cursor --cursor-root <ide-workspace> --json");
    console.log("  # Default: MCP + portable skills (intent-loop, …). Opt out: --no-skills");
    console.log("  # Omit --project; pass tool arg project=<this dir>. See GETTING_STARTED.md");
    console.log("");
...
program
    ...
    if (opts.json) {
        ...
        if (legacyRoles) {
            console.warn("Warning: legacy certifications/roles/ present — migrate to certifications/profiles/");
        }
    }
    process.exitCode = 0;
});
program
    .command("about")
    .description("Product identity: methodology-as-specs эталон (ontology→content→profiles→coverage/export), assessment+learning; not LMS / not agent harness; competency owners; version, capabilities (no project required)")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
...
program
    .command("suggest")
    .description("Next methodology actions from project state (export / player / gaps) with Russian levers")
...
player
    .command("sync")
    .description("Install or refresh player/ from SDM templates (does not touch methodology YAML)")
...
studio
    .command("sync")
    .description("Install or refresh studio/ from SDM templates (does not touch methodology YAML)")
...
studio
    .command("push-view")
    .description("Write sdm.studio.view/v1 JSON into .sdm/studio/current-view.json (bridge)")
...
studio
    ...
    .description("Build coverage/пробелы view from cert gaps (+ suggest) into bridge current-view.json")
    ...
    try {
        ...
        if (opts.json) {
            ...
            console.log(`  пробелы: не покрыто=${result.summary.missing}, слабо покрыто=${result.summary.thin}, покрыто=${result.summary.ok}`);
        }
        process.exitCode = 0;
    }
    catch (err) {
...
studio
    .command("pull-action")
    .description("Read latest action from .sdm/studio/last-action.json (bridge)")
...
studio
    .command("serve")
    .description("Localhost static studio/ + bridge API (127.0.0.1; no methodology writes)")
...
program
    ...
    try {
        ...
        const bad = run.document.coverage?.hasMissing ||
...
quality
    .command("report")
    .description("Сводный отчёт качества (methodology / corpus / diff): вердикт, матрица ●○○, глоссарий")
...
program
    ...
    .action((query, opts) => {
    try {
        if (opts.kind && opts.kind !== "skill" && opts.kind !== "question")
            throw new SdmError("VALIDATION_FAILED", "--kind must be skill or question");
        ...
        if (opts.json) {
            ...
            console.log(run.hits.map((hit) => `${hit.score}\t${hit.kind}\t${hit.id}`).join("\n"));
        }
        process.exitCode = 0;
    }
    catch (err) {
...
profile
    ...
    try {
        ...
        if (opts.json) {
            ...
            console.log("");
            console.log(`Next: sdm skill add … then sdm cert create --profile ${result.profile.profile} --level …`);
        }
        process.exitCode = 0;
    }
    catch (err) {
...
cert
    ...
    .option("--requirement <skill:depth[:weight]>", "Requirement: skill:depth or skill:depth:weight (repeatable). Depth can be 0..1 or junior/middle/senior/expert.", (value, prev) => [...prev, value], [])
    .option("--threshold <n>", "Pass threshold 0..1", (v) => Number.parseFloat(v))
    .option("--force", "Overwrite existing level file", false)
    .option("--no-normalize-weights", "Fail if requirement weights do not already sum to 1")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
        ...
        if (opts.json) {
            ...
            console.log("");
            console.log(`Next: sdm cert coverage --profile ${result.profile.profile} --level ${result.level.level}`);
        }
        process.exitCode = 0;
    }
    catch (err) {
...
cert
    .command("patch")
    .description("Patch an existing certification level (add/set/remove requirements, metadata)")
    .requiredOption("--level <id>", "Level id to patch")
    .option("--profile <id>", "Optional profile id (consistency check against level.profile)")
    .option("--add-requirement <skill:depth:weight>", "Add requirement (fails if skill already present; repeatable)", (value, prev) => [...prev, value], [])
    .option("--set-requirement <skill:depth:weight>", "Upsert requirement by skill id (repeatable)", (value, prev) => [...prev, value], [])
    .option("--remove-requirement <skill>", "Remove requirement by skill id (repeatable)", (value, prev) => [...prev, value], [])
    .option("--from <skill:amount>", "Transfer weight from donor when adding (repeatable)", (value, prev) => [...prev, value], [])
    .option("--absorb-into <skill>", "When removing, add removed weight onto this skill")
...
cert
    .command("reweight")
    .description("Adjust requirement weights on a level (transfer or full --set map; sum must stay 1)")
    ...
    .option("--from <skill[:amount]>", "Donor skill (skill or skill:amount; repeatable)", (value, prev) => [...prev, value], [])
    .option("--set <skill=weight>", "Full weight map entry (repeatable; must cover every skill)", (value, prev) => [...prev, value], [])
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
...
cert
    .command("coverage")
    .description("Report how well the question library covers a certification level (profile + level)")
    ...
    try {
        ...
        if (opts.json) {
            ...
            console.log(`Mode: ${result.coverageMode} — ok ≥ ${result.minOkQuestions} questions/skill AND depthRatio≥0.9` +
                (result.coverageMode === "blueprint" ? " (+ topics/types blueprint)" : ""));
            console.log("");
            for (const skill of result.skills) {
                console.log(`${skill.statusSymbol} ${skill.skill}` +
...
cert
    .command("gaps")
    .description("List certification skills that are missing or thin (agent-friendly subset of coverage)")
    .requiredOption("--profile <profile>", "Profile id (e.g. java-developer)")
    .requiredOption("--level <level>", "Level id (e.g. middle)")
    .option("--team <team>", "Optional team id (requirement overlays)")
    .option("--json", "Machine-readable JSON output for agents", false)
    ...
    try {
        ...
        if (opts.json) {
            ...
            for (const q of summary) {
                console.log(`- ${q.id}  skill=${q.skill}  type=${q.type}  difficulty=${q.difficulty}`);
            }
            if (warnings.length > 0) {
                console.log("");
...
question
    .command("generate")
    .description("Build agent-facing draft shells + context for a skill (does not write files)")
    ...
    .option("--type <type>", "Homogeneous draft type: single_choice | multi_choice | code | open (mutually exclusive with --mix)")
    .option("--mix <preset>", "Type-mix preset: single | mixed | full (mutually exclusive with --type)")
    ...
    try {
        ...
        if (opts.json) {
            ...
            if (result.context.gap) {
                console.log(`Gap: ${result.context.gap.status} (${result.context.gap.questionCount} questions)`);
            }
            console.log("");
            ...
            for (const d of result.drafts) {
                console.log(`- #${d.index} type=${d.type} difficulty=${d.difficulty} text=${d.text}`);
            }
            console.log("");
...
question
    ...
    .requiredOption("--type <type>", "Question type: single_choice | multi_choice | code | open")
    .requiredOption("--difficulty <n>", "Difficulty 0..1", parseFloat)
    .requiredOption("--text <text>", "Question text")
    .option("--option <text>", "Answer option (repeatable; required for choice types)", (value, prev) => [...prev, value], [])
    .option("--correct <n>", "Correct option index, 1-based (repeatable for multi_choice)", (value, prev) => [...prev, Number.parseInt(value, 10)], [])
    .option("--explanation <text>", "Explanation shown after answering")
    .option("--code-template <text>", "Code template for type=code")
    .option("--expected <text>", "Expected short answer for type=open (repeatable)", (value, prev) => [...prev, value], [])
    .option("--topic <topic>", "Topic tag (repeatable)", (value, prev) => [...prev, value], [])
    .option("--evidence <kind>", "Probe evidence: knowledge | skill | artifact (open/code)")
    .option("--min-depth <n>", "Probe min_depth 0..1 (open/code)", parseFloat)
    .option("--red-flag <text>", "Probe red flag (repeatable)", (value, prev) => [...prev, value], [])
    .option("--rubric <score:text>", "Rubric row score:description (repeatable, score 0-3)", (value, prev) => [...prev, value], [])
    ...
    try {
        ...
        if (!allowed.includes(opts.type)) {
            throw new SdmError("VALIDATION_FAILED", `Invalid --type "${opts.type}". Expected: ${allowed.join(", ")}`);
        }
        let correct;
        ...
        const projectRoot = findProjectRoot(process.cwd());
        const evidenceKinds = ["knowledge", "skill", "artifact"];
        if (opts.evidence &&
            !evidenceKinds.includes(opts.evidence)) {
            throw new SdmError("VALIDATION_FAILED", `Invalid --evidence "${opts.evidence}". Expected: ${evidenceKinds.join(", ")}`);
        }
    ...
    try {
        ...
        if (opts.json) {
            ...
            for (const t of summary) {
                console.log(`- ${t.id}  ${t.term}  kind=${t.kind}  skills=${t.skills.join(",") || "—"}`);
            }
        }
    }
    catch (err) {
...
term
    ...
    .option("--alias <text>", "Alias (repeatable)", (value, prev) => [...prev, value], [])
    .option("--skill <skillId>", "Linked skill id (repeatable)", (value, prev) => [...prev, value], [])
    .option("--kind <kind>", "concept | product", "concept")
    .option("--force", "Overwrite existing term file", false)
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((id, opts) => {
    try {
        const kinds = ["concept", "product"];
        if (!kinds.includes(opts.kind)) {
            throw new SdmError("VALIDATION_FAILED", `Invalid --kind "${opts.kind}". Expected: concept, product`);
        }
        const projectRoot = findProjectRoot(process.cwd());
...
topic
    .command("registry")
    .description("Report topic registry status: registered topics, slugs from skills/questions/courses missing from registry, orphans, unused")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
...
topic
    .command("sync")
    .description("Back-fill library/topics YAML for every slug in skill.topics (idempotent)")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
...
topic
    .command("apply")
    .description("Merge registry labels into skill.topic_labels (one-shot enrichment)")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
...
course
    .command("heal")
    .description("Auto-fill topic registry, back-fill skill topics from lessons, suggest descriptions from lesson content. Run after filling lesson bodies but before re-export.")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action(async (opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
        ...
        const files = readdirSync(exportsDir).filter((f) => f.startsWith("course-") && f.endsWith(".json")).sort();
        if (files.length === 0) {
            throw new SdmError("COURSE_HEAL_NO_EXPORT", "No course export file found under exports/. Export a filled course pack first via export learning.");
        }
...
skill
    ...
    .option("--topic <topic>", "Expected topic tag (repeatable)", (value, prev) => [...prev, value], [])
...
skill
    .command("impact")
    .description("Show skills/profiles/levels/questions/exports affected by changing a skill")
    ...
    try {
        ...
        if (opts.json) {
            ...
            console.log(`  downstream skills: ${d.downstreamSkills.join(", ") || "—"}`);
            console.log(`  profiles: ${d.profiles.join(", ") || "—"}`);
            console.log(`  levels: ${d.levels.map((l) => l.level).join(", ") || "—"}`);
            console.log(`  questions: ${d.questions.length} (${d.questions.map((q) => q.id).slice(0, 8).join(", ")}${d.questions.length > 8 ? ", …" : ""})`);
            console.log(`  exports: ${d.exports.length}${d.exports.length
...
content
    .command("stale")
    .description("List questions/exports with missing or mismatched meta.basis (content freshness)")
        ...
        if (opts.json) {
            ...
            if (d.stale.length === 0) {
                ...
                for (const item of d.stale) {
                    ...
                    console.log(`  [${item.severity}] ${item.kind} ${key} — ${item.reason}`);
                }
            }
...
exportCmd
    ...
    .option("--seed <n>", "RNG seed for adaptive sampling and/or --shuffle-options", (v) => Number.parseInt(v, 10), 42)
    .option("--per-skill <n>", "Questions per skill when adaptive", (v) => Number.parseInt(v, 10), 3)
    .option("--shuffle-options", "Permute choice options and remap correct (deterministic with --seed)", false)
    .option("--include-type <type>", "Allowlist question type (repeatable; mutually exclusive with --exclude-type)", (value, prev) => [...prev, value], [])
    .option("--exclude-type <type>", "Denylist question type (repeatable; mutually exclusive with --include-type)", (value, prev) => [...prev, value], [])
    .option("--include-skill <id>", "Allowlist skill id (repeatable; mutually exclusive with --exclude-skill)", (value, prev) => [...prev, value], [])
    .option("--exclude-skill <id>", "Denylist skill id (repeatable; mutually exclusive with --include-skill)", (value, prev) => [...prev, value], [])
    .option("--include-question <id>", "Allowlist question id (repeatable; after skill/type filters)", (value, prev) => [...prev, value], [])
    .option("--json", "Agent envelope { ok, format, document }", false)
    .option("--raw", "Print the bare consumer document (schemaVersion at top level), no envelope", false)
    .action((opts) => {
    try {
...
function registerExportLearningCommand(name, description) {
    exportCmd
        .command(name)
        .description(description)
        .option("--profile <profile>", "Profile id (with --level)")
        .option("--level <level>", "Level id (with --profile)")
        .option("--from-gaps", "Modules only for missing/thin skills (needs profile/level)", false)
        .option("--skill <id>", "Single skill pack")
        .option("--topic <topic>", "Filter by skill/question topic tag")
        .option("--from-questions <ids>", "Comma-separated question ids (teach under these anchors)")
        .option("--depth <depth>", "Content depth: brief|standard|detailed", "standard")
        .option("--format <format>", "Artifact format: howto|notes|cheatsheet|course (concept→notes deprecated)", "howto")
        .option("--no-practice", "Omit practiceQuestionIds")
        .option("--locale <locale>", "Optional locale hint for agent prose (e.g. ru)")
        .option("--strict-context", "Fail when depth=detailed and critical TeachingContext warnings exist", false)
...
registerExportLearningCommand("learning", "Export educational materials (TeachingContext + modules + practice ids; not an LMS)");
registerExportLearningCommand("course", "Alias of export learning (TeachingContext + modules + practice ids; not an LMS)");
exportCmd
    .command("kit")
    .description("Export expert interview kit (skill cards + open/code probes + glossary; HTML is render only)")
    .requiredOption("--profile <profile>", "Profile id")
    .requiredOption("--level <level>", "Level id")
    .option("--format <format>", "Output format: json|html", "json")
    .option("--strict", "Fail when kit-readiness warnings exist (missing probes, explanations)", false)
    .option("--out <path>", "Write output to file (creates parent dirs). Default: stdout. Convention: exports/kit-<profile>-<level>.html")
    .option("--json", "Agent envelope { ok, document, warnings }", false)
    .option("--raw", "Print the bare consumer document (schemaVersion at top level), no envelope", false)
    .action((opts) => {
    try {
        ...
        const fileBody = run.format === "html" && run.html
            ? run.html
...
mcp
    .command("config")
    .description("Print MCP config with resolved absolute paths (no hand-edited paths)")
    ...
    .option("--cursor-root <dir>", "Directory that owns .cursor/ (for host=cursor)")
    .option("--gigacode-home <dir>", "Directory for GigaCode settings.json (default: ~/.gigacode)")
    .option("--config <file>", "Override recommended config path")
    .action((opts) => {
    try {
        const host = opts.host;
        if (!listHosts().some((h) => h.id === host)) {
            throw new SdmError("UNKNOWN_HOST", `Unknown MCP host "${opts.host}". Run: sdm mcp hosts`);
        }
        ...
        if (opts.json) {
            ...
            console.log("projectRoot: (none — one MCP for many projects; pass tool arg `project`)");
            console.log("");
...
mcp
    .command("install")
    .description("Write/merge SDM MCP into host configs; also install portable skills by default")
    .option("--hosts <list>", "Host ids: comma-separated or all (e.g. cursor,gigacode,multitool)")
    .option("--cursor", "Alias for --hosts cursor", false)
    .option("--cursor-root <dir>", "Directory that owns .cursor/ (default: cwd; use workspace root in monorepos)")
    .option("--gigacode-home <dir>", "Directory for GigaCode settings.json (default: ~/.gigacode)")
    .option("--config <file>", "Override config path for selected host(s)")
    .option("--project <dir>", "Optional default SDM_PROJECT_ROOT (omit for multi-project; prefer tool arg `project`)")
    .option("--name <name>", "MCP server key (host sidebar label)", DEFAULT_MCP_SERVER_NAME)
    .option("--no-skills", "Skip portable skills install (MCP config only)")
    .option("--agents-root <dir>", "Override path to SDM agents/ when installing skills")
    ...
    try {
        ...
        if (opts.skills !== false) {
            try {
                ...
                if (opts.json) {
                    const code = skillsErr instanceof SdmError ? skillsErr.code : "UNEXPECTED";
                    const message = skillsErr instanceof Error
                        ? skillsErr.message
                        : String(skillsErr);
                    console.log(JSON.stringify({
                        ok: false,
                    ...
                    console.error(`Wrote SDM MCP, but skills install failed.`);
        ...
        if (opts.json) {
            ...
            console.log(`  SDM_PROJECT_ROOT: ${mcp.config.projectRoot ?? "(not set — pass tool arg project per methodology)"}`);
            if (skills) {
                console.log(`  skills: ${skills.skillIds.join(", ")} (${skills.installs.filter((i) => i.mode !== "skipped").length} wrote, ${skills.installs.filter((i) => i.mode === "skipped").length} skipped)`);
            }
            else {
                console.log("  skills: (skipped — --no-skills)");
            }
            console.log("");
            console.log("Reload MCP and skills / restart agent host, then try tool: doctor (pass project per methodology)");
        }
        process.exitCode = 0;
    }
    catch (err) {
...
agent
    .command("install")
    .description("Mirror SDM agents/*/SKILL.md into host skill folders (Cursor, GigaCode, …)")
    .option("--hosts <list>", "Host ids: comma-separated or all (e.g. cursor,gigacode,multitool)")
    .option("--cursor-root <dir>", "Directory that owns .cursor/skills (default: cwd)")
    .option("--gigacode-home <dir>", "Directory for GigaCode skills (default: ~/.gigacode)")
    .option("--agents-root <dir>", "Override path to SDM agents/ (default: package / SDM_HOME)")
    .option("--link", "Symlink skill dirs instead of copying (dev)", false)
...
intent
    ...
    try {
        ...
        if (opts.json) {
            ...
            console.log(`OK: intent plan kind=${result.plan.kind} profile=${result.plan.profile.id} level=${result.plan.level.id}`);
        }
        process.exitCode = 0;
    }
    catch (err) {
...
skill
    ...
    .description("Suggest related_to/depends_on edges from skill descriptions and topics (dry-run; confirm via skill link)")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
        ...
        if (opts.json) {
            ...
            for (const e of edges) {
                console.log(`  ${e.from} ~ ${e.to}  (${e.kind}, conf=${e.confidence})  — ${e.reason}`);
            }
            if (edges.length > 0) {
                console.log("");
...
question
    ...
    .description("Validate the whole question library for type/payload consistency (options, correct, expected)")
    .option("--json", "Machine-readable JSON output for agents", false)
    .action((opts) => {
    try {
        const projectRoot = findProjectRoot(process.cwd());
...
function readStdin() {
    return readStdinWithCode("INTENT_PLAN_INVALID", "No plan on stdin. Pass --file <path> or pipe JSON.");
}
function readStdinForStudio(emptyMessage) {
    return readStdinWithCode("STUDIO_VIEW_INVALID", emptyMessage);
}
function readStdinWithCode(code, emptyMessage) {
