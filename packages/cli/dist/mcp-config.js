...
export function resolveMcpPackageJson() {
...
function resolveMcpEntry() {
    ...
    throw new SdmError("MCP_NOT_FOUND", "Could not resolve @spec-driven-methodology/mcp dist/index.js. Run `npm run build -w @spec-driven-methodology/mcp` (and prefer `npm link -w @spec-driven-methodology/mcp`).");
}
/**
 ...
 */
export function getResolvedMcpVersion() {
    const pkgJson = resolveMcpPackageJson();
    if (!pkgJson)
        return null;
    try {
 ...
 */
export function formatSdmVersionOutput(cliVersion) {
 ...
 */
export function mergeMcpServersJson(configPath, serverName, mcpServer) {
    ...
    if (existsSync(path)) {
        try {
            ...
            throw new SdmError("VALIDATION_FAILED", `Cannot parse existing MCP config: ${path}`);
        }
    }
 ...
 */
export function mergeMcpIntoOpenCode(configPath, serverName, mcpServer) {
    const path = resolve(configPath);
    mkdirSync(dirname(path), { recursive: true });
    let existing = {};
    if (existsSync(path)) {
        try {
            ...
            throw new SdmError("VALIDATION_FAILED", `Cannot parse existing OpenCode MCP config: ${path}`);
        }
    }
    const prevServers = existing.mcp &&
