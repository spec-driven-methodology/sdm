...
export function detectShell(envShell = process.env.SHELL ?? "") {
    if (envShell.includes("fish"))
        return "fish";
    if (envShell.includes("bash"))
        return "bash";
    return "zsh";
}
function renderZsh(bin, tree) {
    const top = tree
...
function renderFish(bin, tree) {
    ...
    for (const c of tree) {
        lines.push(`complete -c ${bin} -n "__fish_use_subcommand" -a "${c.name}" -d "${shellWords(c.description)}"`);
        for (const s of c.subcommands) {
            lines.push(`complete -c ${bin} -n "__fish_seen_subcommand_from ${c.name}" -a "${s.name}" -d "${shellWords(s.description)}"`);
            for (const d of s.subcommands) {
                lines.push(`complete -c ${bin} -n "__fish_seen_subcommand_from ${c.name}; and __fish_seen_subcommand_from ${s.name}" -a "${d.name}" -d "${shellWords(d.description)}"`);
            }
        }
    }
    return `${lines.join("\n")}\n`;
}
export function renderCompletionScript(shell, program) {
...
export function parseShellArg(raw) {
...
function completionsDir(home) {
    return join(home, ".sdm", "completions");
}
export function completionScriptPath(shell, home = homedir()) {
...
export function shellRcPath(shell, home = homedir()) {
    switch (shell) {
        case "zsh":
...
function rcSourceBlock(shell, scriptPath) {
    if (shell === "fish")
        return "";
    return [
        BEGIN_MARK,
...
export function stripLegacyCompletionLines(rcText) {
    return rcText
        .split("\n")
        .filter((line) => {
        const t = line.trim();
        return !(t === 'eval "$(sdm completion zsh)"' ||
 ...
 */
export function upsertRcBlock(rcPath, block, dryRun = false) {
...
function shouldSkipInstall(respectSkipEnv) {
    if (!respectSkipEnv)
        return null;
 ...
 */
export function installCompletion(program, options = {}) {
...
export function registerCompletion(program) {
    ...
    completion
        .command("install")
        .description("Install/update shell completion (writes ~/.sdm/completions + hooks rc). Safe to re-run after build/link.")
        ...
        .option("--from-hook", "Build/link hook mode: skip on CI / SDM_SKIP_COMPLETION=1; never fail the parent script", false)
        .action((opts) => {
        try {
            ...
            if (opts.json) {
                ...
                console.log(`[sdm] completion updated (${result.shell}) → ${result.scriptPath}`);
            }
        }
        catch (err) {
