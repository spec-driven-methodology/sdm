...
import { z } from "zod";
import { addQuestion, addSkill, appendActionLog, buildAbout, buildSuggest, createCertification, createProfile, patchCertification, reweightCertification, exportDocumentPayload, exportConfluence, exportCourse, exportKit, exportMatrix, exportMermaid, exportTest, findProjectRoot, getProductVersion, initMethodologyProject, isMethodologyProject, hasLegacyRolesDirectory, locateProject, listMethodologyProjects, syncPlayerAssets, syncStudioAssets, pushStudioView, pushStudioCoverage, pullStudioAction, linkSkill, listQuestions, listTerms, addTerm, parseSkillIdList, rebuildSemanticIndex, searchSemanticIndex, runMethodologyAudit, buildQualityReport, runCertCoverage, runCertGaps, runSkillGraph, runSkillImpact, runContentStale, generateQuestions, validateQuestion, validateQuestionLibraryDeep, inferEdgesForProject, buildTopicRegistry, syncTopicRegistryFromSkills, healCourseWarnings, SdmError, } from "@spec-driven-methodology/core";
/** Resolve methodology start dir: explicit project → SDM_PROJECT_ROOT → cwd. */
export function resolveStartDir(project) {
...
export async function loggedTool(name, args, fn) {
    const started = Date.now();
    const projectArg = args && typeof args === "object" && args !== null && "project" in args
        ? args.project
        : undefined;
    ...
    try {
        ...
        const code = err instanceof SdmError
            ? err.code
            : "UNEXPECTED";
        const message = err instanceof Error ? err.message : String(err);
        appendActionLog({
...
export async function toolAbout(_args = {}) {
    try {
...
export async function toolCourseHeal(args) {
    try {
        ...
        let modules = [];
        let warnings = [];
        if (readdirSync(exportsDir).some((f) => f.startsWith("course-") && f.endsWith(".json"))) {
            ...
            modules = (doc.document?.modules ?? doc.modules ?? []);
            warnings = (doc.document?.warnings ?? doc.warnings ?? []);
        }
...
export async function toolIndexRebuild(args = {}) {
    try {
...
export async function runTool(name, args = {}) {
    switch (name) {
        case "about":
            return toolAbout(args);
        case "suggest":
            return toolSuggest(args);
        case "doctor":
            return toolDoctor(args);
        case "audit":
            return toolAudit(args);
        case "quality_report":
            return toolQualityReport(args);
        case "init":
            return toolInit(args);
        case "locate_project":
            return toolLocateProject(args);
        case "list_projects":
            return toolListProjects(args);
        case "player_sync":
            return toolPlayerSync(args);
        case "studio_sync":
            return toolStudioSync(args);
        case "studio_push_view":
            return toolStudioPushView(args);
        case "studio_push_coverage":
            return toolStudioPushCoverage(args);
        case "studio_pull_action":
            return toolStudioPullAction(args);
        case "skill_add":
            return toolSkillAdd(args);
        case "skill_link":
            return toolSkillLink(args);
        case "skill_graph":
            return toolSkillGraph(args);
        case "skill_impact":
            return toolSkillImpact(args);
        case "skill_suggest_links":
            return toolSkillSuggestLinks(args);
        case "content_stale":
            return toolContentStale(args);
        case "profile_create":
            return toolProfileCreate(args);
        case "cert_create":
            return toolCertCreate(args);
        case "cert_patch":
            return toolCertPatch(args);
        case "cert_reweight":
            return toolCertReweight(args);
        case "cert_coverage":
            return toolCertCoverage(args);
        case "cert_gaps":
            return toolCertGaps(args);
        case "question_add":
            return toolQuestionAdd(args);
        case "question_validate":
            return toolQuestionValidate(args);
        case "question_list":
            return toolQuestionList(args);
        case "question_deep_validate":
            return toolQuestionDeepValidate(args);
        case "term_add":
            return toolTermAdd(args);
        case "term_list":
            return toolTermList(args);
        case "question_generate":
            return toolQuestionGenerate(args);
        case "export_test":
            return toolExportTest(args);
        case "export_matrix":
            return toolExportMatrix(args);
        case "export_learning":
        case "export_course":
            return toolExportCourse(args);
        case "export_mermaid":
            return toolExportMermaid(args);
        case "export_confluence":
            return toolExportConfluence(args);
        case "export_kit":
            return toolExportKit(args);
        case "index_rebuild":
            return toolIndexRebuild(args);
        case "search":
            return toolSearch(args);
        case "topic_registry":
            return toolTopicRegistry(args);
        case "topic_sync":
            return toolTopicSync(args);
        case "course_heal":
            return toolCourseHeal(args);
        default: {
...
const projectParam = z
    .string()
    .optional()
    .describe("Methodology project directory (multi-project). Default: SDM_PROJECT_ROOT or cwd.");
...
const exportLearningParams = {
    ...
    format: z
        ...
        .optional()
        .describe("howto=Инструкция, notes=Конспект, cheatsheet=Шпаргалка, course=Курс; concept→notes deprecated"),
    includePractice: z
        .boolean()
        .optional()
...
const exportKitParams = {
    ...
    format: z
        .enum(["json", "html"])
        .optional()
        .describe("json=kit document in document field (default); html=also returns html field with self-contained page"),
    strict: z
        .boolean()
        .optional()
...
export const TOOL_INPUT_SHAPES = {
    ...
    init: {
        ...
        targetDir: z
            .string()
            .optional()
            .describe("Target directory (default: project / SDM_PROJECT_ROOT / cwd)"),
    ...
    locate_project: {
        dir: z
            .string()
            .describe("Directory to start from (workdir). Walks up to the nearest sdm.yaml."),
    },
    list_projects: {
        workspaceDir: z
            .string()
    ...
    question_generate: {
        ...
        mix: z
            ...
            .optional()
            .describe("Type rotation: single|mixed|full (mutually exclusive with type)"),
        profile: profileOptParam,
        level: levelOptParam,
    },
    export_test: {
        ...
        includeTypes: questionTypeFilterParam.describe("Allowlist question types (mutually exclusive with excludeTypes)"),
        excludeTypes: questionTypeFilterParam.describe("Denylist question types (mutually exclusive with includeTypes)"),
        includeSkills: z
            .array(z.string())
            .optional()
...
export const TOOL_DESCRIPTIONS = {
    about: "Product identity: methodology-as-specs эталон (ontology→content→profiles→coverage/export), assessment+learning; not LMS / not agent harness; competency owners; version, capabilities (no project required)",
    suggest: "Next methodology actions from project state (export / player / gaps / quality) with Russian levers",
    doctor: "Confirm a SDM methodology project (pass project for multi-project MCP)",
    audit: "Detailed live audit: ontology, library duplicates, optional coverage (sibling of quality_report summary)",
    quality_report: "Summary quality report (methodology or corpus): verdict, ●○○ matrix, glossary; optional save/diff. save defaults true.",
    init: "Initialize a SDM methodology project in the target directory",
    locate_project: "Locate the nearest SDM methodology project from a directory (walk up to sdm.yaml). Returns root + name. Multi-project: use this to resolve which project a path belongs to, then pass project to other tools.",
    list_projects: "List SDM methodology projects (sdm.yaml) under a workspace directory. Multi-project: discover candidates to operate on.",
    player_sync: "Install or refresh player/ author preview assets (methodology YAML untouched)",
    studio_sync: "Install or refresh studio/ Methodology Studio assets (methodology YAML untouched)",
    studio_push_view: "Write sdm.studio.view/v1 JSON into .sdm/studio/current-view.json (bridge)",
    studio_push_coverage: "Build coverage/пробелы studio view from cert gaps (+ suggest) into bridge",
    studio_pull_action: "Read latest studio action from bridge (.sdm/studio/last-action.json)",
    skill_add: "Create an ontology skill YAML (id, name, optional category/topics); use skill_link for depends_on/related_to",
    skill_link: "Link depends_on / related_to on an existing skill",
    skill_graph: "Show skill dependency tree for a profile/level with optional coverage",
    skill_impact: "List skills/profiles/levels/questions/exports affected by changing a skill",
    content_stale: "List questions/exports with missing or mismatched meta.basis (content freshness)",
    profile_create: "Create a certification profile with empty levels[]",
    cert_create: "Create level certification for an existing profile with requirement triples",
    cert_patch: "Patch an existing certification level requirements/metadata (add/set/remove + weight transfer)",
    cert_reweight: "Transfer or replace requirement weights on a level (sum must stay 1)",
    cert_coverage: "Full certification coverage report (status per skill; optional team; blueprint workItems when enabled)",
    cert_gaps: "Certification gaps only (missing/thin skills + workItems in blueprint mode)",
    question_add: "Add a validated question bound to a skill (choice: options+correct; open: expected); honors writeGate",
    question_validate: "Dry-run validate a question draft (no writes; agent rewrite loop before question_add)",
    ...
    question_generate: "Build draft question shells + agent context for a skill (does not write; use type XOR mix)",
    export_test: "Export a certification test package (json/csv); optional adaptive, shuffle, type/skill/question filters",
    export_matrix: "Export a profile competency matrix (levels × skills)",
    export_learning: "Export educational materials (TeachingContext + modules + practice ids; not an LMS). Formats: howto|notes|cheatsheet|course. Agent fills lesson prose.",
    export_course: "Alias of export_learning — educational materials pack (not an LMS). Prefer export_learning.",
    export_mermaid: "Export a Mermaid skill graph with optional coverage coloring",
    export_confluence: "Export a Confluence-ready Markdown page (coverage + mermaid + matrix)",
    export_kit: "Expert interview kit (sdm.export.kit/v1): open/code probes, glossary, checklist for profile+level. format=json (default) returns document; format=html returns self-contained page in html field — write to exports/kit-{profile}-{level}.html, do not hand-edit. Not learner cheatsheet (use export_learning). HTML is render only — SSOT is YAML.",
    index_rebuild: "Rebuild offline semantic index (requires search.provider=lancedb in sdm.yaml)",
    search: "Search the offline semantic index (requires prior index_rebuild with lancedb provider)",
    skill_suggest_links: "Suggest related_to/depends_on edges from skill descriptions + topics (dry-run; apply via skill_link)",
    question_deep_validate: "Validate the whole question library for type/payload consistency (choice options+correct, open expected/rubric) + difficulty summary; errors block, findings advisory",
    topic_registry: "Report project-wide topic registry: registered topics (library/topics), slugs from skills/questions/courses missing from registry, orphans, unused",
    topic_sync: "Back-fill library/topics YAML for every slug in skill.topics (idempotent; closes SKILL_TOPICS_EMPTY source)",
    course_heal: "Auto-heal course warnings: registry sync + back-fill skill topics from lesson topics + suggest descriptions from lesson content (no LLM; closest SKILL_TOPICS_EMPTY/SKILL_DESCRIPTION_THIN); re-run export_learning after",
};
/** Non-empty Zod `.description` for a field (tests / hygiene). */
export function zodFieldDescription(schema) {
    let cur = schema;
    const seen = new Set();
...
export function registerTools(server) {
    server.tool("about", TOOL_DESCRIPTIONS.about, TOOL_INPUT_SHAPES.about, async (args) => loggedTool("about", args, () => toolAbout(args)));
    server.tool("suggest", TOOL_DESCRIPTIONS.suggest, TOOL_INPUT_SHAPES.suggest, async (args) => loggedTool("suggest", args, () => toolSuggest(args)));
    server.tool("doctor", TOOL_DESCRIPTIONS.doctor, TOOL_INPUT_SHAPES.doctor, async (args) => loggedTool("doctor", args, () => toolDoctor(args)));
    server.tool("player_sync", TOOL_DESCRIPTIONS.player_sync, TOOL_INPUT_SHAPES.player_sync, async (args) => loggedTool("player_sync", args, () => toolPlayerSync(args)));
    server.tool("studio_sync", TOOL_DESCRIPTIONS.studio_sync, TOOL_INPUT_SHAPES.studio_sync, async (args) => loggedTool("studio_sync", args, () => toolStudioSync(args)));
    server.tool("studio_push_view", TOOL_DESCRIPTIONS.studio_push_view, TOOL_INPUT_SHAPES.studio_push_view, async (args) => loggedTool("studio_push_view", args, () => toolStudioPushView(args)));
    server.tool("studio_push_coverage", TOOL_DESCRIPTIONS.studio_push_coverage, TOOL_INPUT_SHAPES.studio_push_coverage, async (args) => loggedTool("studio_push_coverage", args, () => toolStudioPushCoverage(args)));
    server.tool("studio_pull_action", TOOL_DESCRIPTIONS.studio_pull_action, TOOL_INPUT_SHAPES.studio_pull_action, async (args) => loggedTool("studio_pull_action", args, () => toolStudioPullAction(args)));
    server.tool("audit", TOOL_DESCRIPTIONS.audit, TOOL_INPUT_SHAPES.audit, async (args) => loggedTool("audit", args, () => toolAudit(args)));
    server.tool("quality_report", TOOL_DESCRIPTIONS.quality_report, TOOL_INPUT_SHAPES.quality_report, async (args) => loggedTool("quality_report", args, () => toolQualityReport(args)));
    server.tool("init", TOOL_DESCRIPTIONS.init, TOOL_INPUT_SHAPES.init, async (args) => loggedTool("init", args, () => toolInit(args)));
    server.tool("locate_project", TOOL_DESCRIPTIONS.locate_project, TOOL_INPUT_SHAPES.locate_project, async (args) => loggedTool("locate_project", args, () => toolLocateProject(args)));
    server.tool("list_projects", TOOL_DESCRIPTIONS.list_projects, TOOL_INPUT_SHAPES.list_projects, async (args) => loggedTool("list_projects", args, () => toolListProjects(args)));
    server.tool("skill_add", TOOL_DESCRIPTIONS.skill_add, TOOL_INPUT_SHAPES.skill_add, async (args) => loggedTool("skill_add", args, () => toolSkillAdd(args)));
    server.tool("skill_link", TOOL_DESCRIPTIONS.skill_link, TOOL_INPUT_SHAPES.skill_link, async (args) => loggedTool("skill_link", args, () => toolSkillLink(args)));
    server.tool("skill_graph", TOOL_DESCRIPTIONS.skill_graph, TOOL_INPUT_SHAPES.skill_graph, async (args) => loggedTool("skill_graph", args, () => toolSkillGraph(args)));
    server.tool("skill_impact", TOOL_DESCRIPTIONS.skill_impact, TOOL_INPUT_SHAPES.skill_impact, async (args) => loggedTool("skill_impact", args, () => toolSkillImpact(args)));
    server.tool("content_stale", TOOL_DESCRIPTIONS.content_stale, TOOL_INPUT_SHAPES.content_stale, async (args) => loggedTool("content_stale", args, () => toolContentStale(args)));
    server.tool("profile_create", TOOL_DESCRIPTIONS.profile_create, TOOL_INPUT_SHAPES.profile_create, async (args) => loggedTool("profile_create", args, () => toolProfileCreate(args)));
    server.tool("cert_create", TOOL_DESCRIPTIONS.cert_create, TOOL_INPUT_SHAPES.cert_create, async (args) => loggedTool("cert_create", args, () => toolCertCreate(args)));
    server.tool("cert_patch", TOOL_DESCRIPTIONS.cert_patch, TOOL_INPUT_SHAPES.cert_patch, async (args) => loggedTool("cert_patch", args, () => toolCertPatch(args)));
    server.tool("cert_reweight", TOOL_DESCRIPTIONS.cert_reweight, TOOL_INPUT_SHAPES.cert_reweight, async (args) => loggedTool("cert_reweight", args, () => toolCertReweight(args)));
    server.tool("cert_coverage", TOOL_DESCRIPTIONS.cert_coverage, TOOL_INPUT_SHAPES.cert_coverage, async (args) => loggedTool("cert_coverage", args, () => toolCertCoverage(args)));
    server.tool("cert_gaps", TOOL_DESCRIPTIONS.cert_gaps, TOOL_INPUT_SHAPES.cert_gaps, async (args) => loggedTool("cert_gaps", args, () => toolCertGaps(args)));
    server.tool("question_add", TOOL_DESCRIPTIONS.question_add, TOOL_INPUT_SHAPES.question_add, async (args) => loggedTool("question_add", args, () => toolQuestionAdd({
        ...args,
        type: args.type,
    })));
    server.tool("question_validate", TOOL_DESCRIPTIONS.question_validate, TOOL_INPUT_SHAPES.question_validate, async (args) => loggedTool("question_validate", args, () => toolQuestionValidate({
        ...args,
        type: args.type,
    })));
    server.tool("question_list", TOOL_DESCRIPTIONS.question_list, TOOL_INPUT_SHAPES.question_list, async (args) => loggedTool("question_list", args, () => toolQuestionList(args)));
    server.tool("term_add", TOOL_DESCRIPTIONS.term_add, TOOL_INPUT_SHAPES.term_add, async (args) => loggedTool("term_add", args, () => toolTermAdd(args)));
    server.tool("term_list", TOOL_DESCRIPTIONS.term_list, TOOL_INPUT_SHAPES.term_list, async (args) => loggedTool("term_list", args, () => toolTermList(args)));
    server.tool("question_generate", TOOL_DESCRIPTIONS.question_generate, TOOL_INPUT_SHAPES.question_generate, async (args) => loggedTool("question_generate", args, () => toolQuestionGenerate({
        ...args,
        type: args.type,
        mix: args.mix,
    })));
    server.tool("export_test", TOOL_DESCRIPTIONS.export_test, TOOL_INPUT_SHAPES.export_test, async (args) => loggedTool("export_test", args, () => toolExportTest(args)));
    server.tool("export_matrix", TOOL_DESCRIPTIONS.export_matrix, TOOL_INPUT_SHAPES.export_matrix, async (args) => loggedTool("export_matrix", args, () => toolExportMatrix(args)));
    server.tool("export_learning", TOOL_DESCRIPTIONS.export_learning, TOOL_INPUT_SHAPES.export_learning, async (args) => loggedTool("export_learning", args, () => toolExportCourse(args)));
    server.tool("export_course", TOOL_DESCRIPTIONS.export_course, TOOL_INPUT_SHAPES.export_course, async (args) => loggedTool("export_course", args, () => toolExportCourse(args)));
    server.tool("export_mermaid", TOOL_DESCRIPTIONS.export_mermaid, TOOL_INPUT_SHAPES.export_mermaid, async (args) => loggedTool("export_mermaid", args, () => toolExportMermaid(args)));
    server.tool("export_confluence", TOOL_DESCRIPTIONS.export_confluence, TOOL_INPUT_SHAPES.export_confluence, async (args) => loggedTool("export_confluence", args, () => toolExportConfluence(args)));
    server.tool("export_kit", TOOL_DESCRIPTIONS.export_kit, TOOL_INPUT_SHAPES.export_kit, async (args) => loggedTool("export_kit", args, () => toolExportKit(args)));
    server.tool("index_rebuild", TOOL_DESCRIPTIONS.index_rebuild, TOOL_INPUT_SHAPES.index_rebuild, async (args) => loggedTool("index_rebuild", args, () => toolIndexRebuild(args)));
    server.tool("search", TOOL_DESCRIPTIONS.search, TOOL_INPUT_SHAPES.search, async (args) => loggedTool("search", args, () => toolSearch(args)));
    server.tool("skill_suggest_links", TOOL_DESCRIPTIONS.skill_suggest_links, TOOL_INPUT_SHAPES.skill_suggest_links, async (args) => loggedTool("skill_suggest_links", args, () => toolSkillSuggestLinks(args)));
    server.tool("question_deep_validate", TOOL_DESCRIPTIONS.question_deep_validate, TOOL_INPUT_SHAPES.question_deep_validate, async (args) => loggedTool("question_deep_validate", args, () => toolQuestionDeepValidate(args)));
    server.tool("topic_registry", TOOL_DESCRIPTIONS.topic_registry, TOOL_INPUT_SHAPES.topic_registry, async (args) => loggedTool("topic_registry", args, () => toolTopicRegistry(args)));
    server.tool("topic_sync", TOOL_DESCRIPTIONS.topic_sync, TOOL_INPUT_SHAPES.topic_sync, async (args) => loggedTool("topic_sync", args, () => toolTopicSync(args)));
    server.tool("course_heal", TOOL_DESCRIPTIONS.course_heal, TOOL_INPUT_SHAPES.course_heal, async (args) => loggedTool("course_heal", args, () => toolCourseHeal(args)));
}
...
export function createServer(version = getProductVersion()) {
