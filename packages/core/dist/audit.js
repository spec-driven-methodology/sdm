...
import { DEFAULT_DISTRACTOR_QUALITY, evaluateOptionLengthQuality, evaluatePositionBias, } from "./distractor-quality.js";
...
export function findLexicalDuplicates(questions, threshold = 0.85) {
    const dups = [];
...
export function runMethodologyAudit(options) {
    ...
    for (const level of levels) {
        for (const r of level.requirements)
            requiredSkills.add(r.skill);
    }
    const isolatedSkills = [];
    for (const skill of skills) {
        ...
        if (deps === 0 && !referenced)
            isolatedSkills.push(skill.id);
    }
    isolatedSkills.sort();
    const unusedSkills = skills
        ...
        .sort();
    const duplicates = findLexicalDuplicates(questions, options.duplicateThreshold ?? 0.85);
    ...
    try {
        ...
        if (config.search.provider === "lancedb" && existsSync(semanticIndexPath(projectRoot))) {
            const index = JSON.parse(readFileSync(semanticIndexPath(projectRoot), "utf8"));
    ...
    let optionPositionBias;
    let optionLengthOutliers;
    ...
    const biasPriority = policy === "strict" ? "high" : "medium";
    ...
    if (optionLengthOutliers?.length) {
        ...
        const more = optionLengthOutliers.length > 8
...
function formatAuditText(doc) {
    const lines = [];
    lines.push("=== SDM methodology audit ===");
    lines.push("");
    lines.push("Ontology:");
    lines.push(`  Skills: ${doc.ontology.skillCount}`);
    lines.push(`  Isolated: ${doc.ontology.isolatedSkills.join(", ") || "—"}`);
    ...
    if (doc.certifications?.weightSumInvalid.length) {
        ...
        for (const f of doc.certifications.weightSumInvalid) {
            lines.push(`  ⚠️ ${f.level}: sum=${f.sum} (delta ${f.delta}) — use cert reweight`);
        }
    }
    lines.push("");
    lines.push("Library:");
    ...
    if (doc.library.optionPositionBias) {
        ...
        lines.push(`  ⚠️ Option position bias: ${b.firstPositionCount}/${b.sample} (share ${b.share}, threshold ${b.threshold})`);
    }
    if (doc.library.optionLengthOutliers?.length) {
        const ids = doc.library.optionLengthOutliers.slice(0, 12).join(", ");
        const more = doc.library.optionLengthOutliers.length > 12
    ...
    if (doc.coverage) {
        ...
        for (const s of doc.coverage.skills) {
            lines.push(`  ${s.statusSymbol} ${s.skill}: count=${s.questionCount} depth=${s.achievedDepth}/${s.depth} (${s.status})`);
        }
    }
