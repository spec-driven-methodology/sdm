...
import { assertAcyclicDepends, buildSkillGraphFromProject, withProposedDepends, } from "./skill-graph.js";
...
function parseSkillOrThrow(payload) {
    try {
...
export function addSkill(projectRoot, input) {
    ...
    if (existsSync(path) && !input.force) {
        throw new SdmError("SKILL_EXISTS", `Skill file already exists: ${path}. Use --force to overwrite.`);
    }
 ...
 */
export function linkSkill(projectRoot, skillId, input) {
    const dependsOn = uniq(input.dependsOn ?? []);
    const relatedTo = uniq(input.relatedTo ?? []);
    if (dependsOn.length === 0 && relatedTo.length === 0) {
        throw new SdmError("VALIDATION_FAILED", "Provide at least one of --depends-on or --related-to");
    }
    if (dependsOn.includes(skillId)) {
        throw new SdmError("VALIDATION_FAILED", `Skill "${skillId}" cannot depend on itself`);
    }
...
export function parseSkillIdList(raw) {
    if (!raw || !raw.trim()) {
        return [];
    }
