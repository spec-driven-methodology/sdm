...
import { effectiveDistractorMode, loadQualityConfig, } from "./quality-config.js";
import { buildQuestionPayload, normalizeExpectedInput, } from "./question-payload.js";
import { QuestionSchema } from "./schemas.js";
import { assertSkillExists, loadSkill } from "./skills.js";
function pushIssue(target, code, message, severity) {
    target.push({ code, message, severity });
}
function assertChoiceShape(input) {
...
function assertExpectedShape(input) {
 ...
 */
export function validateQuestionDraft(projectRoot, input, options) {
    const errors = [];
    const findings = [];
    try {
    ...
    if (errors.length === 0) {
        try {
            ...
            if (input.expected !== undefined && input.type === "open") {
                payload.expected = normalizeExpectedInput(input.expected);
            }
            question = QuestionSchema.parse(payload);
        }
        catch (err) {
            if (err instanceof ZodError) {
                pushIssue(errors, "VALIDATION_FAILED", err.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join("; "), "error");
            }
            else {
                throw err;
            }
    ...
    if (question) {
        ...
        if (question.type === "open" || question.type === "code") {
            if (!question.evidence) {
                pushIssue(findings, "PROBE_EVIDENCE_MISSING", "Probe questions should declare evidence (knowledge | skill | artifact)", "finding");
            }
            if (!question.rubric || question.rubric.length === 0) {
                pushIssue(findings, "PROBE_RUBRIC_MISSING", "Probe questions should include rubric rows (--rubric 0:…)", "finding");
            }
        }
        // Near-duplicate
        if (writeGate !== "off") {
            const { questions } = loadQuestions(projectRoot);
            const peers = questions.filter((q) => q.skill === question.skill &&
                q.id !== options?.excludeQuestionId &&
                q.id !== question.id);
            let best;
            for (const peer of peers) {
                const similarity = jaccardSimilarity(question.text, peer.text);
                if (similarity >= quality.nearDupThreshold &&
...
export function throwIfValidateErrors(result) {
 ...
 */
export function validateQuestionLibraryDeep(projectRoot) {
    const { questions } = loadQuestions(projectRoot);
    const issues = [];
...
//# sourceMappingURL=question-validate.js.map