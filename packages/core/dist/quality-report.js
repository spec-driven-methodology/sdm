...
import { extractTopicCandidates, scanCorpusSources, } from "./corpus-scan.js";
...
export const QUALITY_REPORT_SCHEMA = "sdm.quality.report/v1";
export const DENSITY_LEVELS = ["full", "partial", "thin", "none"];
export const DENSITY_SYMBOL = {
    full: "●●●",
    partial: "●●○",
    thin: "●○○",
    none: "○○○",
};
export const MATRIX_LEGEND_RU = "●●● плотное · ●●○ есть с дырами · ●○○ тонко · ○○○ почти нет";
export const MATRIX_LEGEND_EN = "●●● dense · ●●○ with gaps · ●○○ thin · ○○○ almost none";
export const BUILTIN_GLOSSARY_RU = [
    {
        term: "SSOT",
...
export const BUILTIN_GLOSSARY_EN = [
    {
        term: "SSOT",
...
export const QualityReportDocumentSchema = z.object({
    ...
    diff: z
        ...
        baselineId: z.string(),
        scoreDelta: z.number(),
        matrixChanges: z.array(z.object({
            row: z.string(),
            col: z.string(),
            from: DensitySchema,
            to: DensitySchema,
...
function makeReportId(mode, scope) {
    const ts = new Date().toISOString().replace(/[:.]/g, "-");
    const bits = [ts, mode];
    if (typeof scope.profile === "string")
        bits.push(slugPart(scope.profile));
    if (typeof scope.level === "string")
        bits.push(slugPart(scope.level));
    if (typeof scope.sourcesDir === "string")
        bits.push("corpus");
...
function coverageStatusToDensity(status) {
    if (status === "ok")
        return "full";
    if (status === "thin")
        return "partial";
...
function questionCountToDensity(n) {
    if (n >= 5)
        return "full";
    if (n >= 3)
        return "partial";
...
function resolveReportsRoot(startDir, sourcesDir) {
    ...
    throw new SdmError("NOT_A_PROJECT", `Not a SDM methodology project: ${start}. Run: sdm init (or pass --sources for corpus mode)`);
}
export function qualityReportsDir(reportsRoot) {
    return reportsRoot;
}
export function loadQualityReport(reportsRoot, reportId) {
    if (!existsSync(reportsRoot)) {
        throw new SdmError("QUALITY_REPORT_NOT_FOUND", `Quality report not found: ${reportId}`);
    }
    const exact = join(reportsRoot, `${reportId}.json`);
    if (existsSync(exact)) {
        return QualityReportDocumentSchema.parse(JSON.parse(readFileSync(exact, "utf8")));
    }
    ...
    return QualityReportDocumentSchema.parse(JSON.parse(readFileSync(join(reportsRoot, match), "utf8")));
}
export function saveQualityReport(reportsRoot, document) {
    mkdirSync(reportsRoot, { recursive: true });
    const path = join(reportsRoot, `${document.id}.json`);
    writeFileSync(path, `${JSON.stringify(document, null, 2)}\n`, "utf8");
    return path;
}
function buildMethodologyReport(projectRoot, options, locale) {
    ...
    let score = 5;
    if (missing > 0)
        score -= Math.min(2, missing);
    if (thin > 0)
        score -= 1;
    if (dupCount > 0)
        score -= 1;
    if (audit.document.ontology.isolatedSkills.length > 2)
        score -= 1;
    ...
    const verdictRu = readiness === "high"
    ...
    const verdictEn = readiness === "high"
...
function topicDensityForCol(topic, col, manifest) {
    const topicRe = new RegExp(topic.split(/\s+/)[0].replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i");
    const relevant = manifest.entries.filter((e) => {
        if (col === "banks" && e.artifactType !== "question_bank")
            return false;
        if (col === "matrices" && e.artifactType !== "matrix" && e.artifactType !== "rubric")
            return false;
        if (col === "program" && e.artifactType !== "program")
            return false;
        if (col === "all" &&
            e.artifactType === "stub")
            return false;
        const hit = topicRe.test(e.path) ||
    ...
    if (hasBank && lines >= 120 && !multi)
        return "full";
    if (hasBank && lines >= 40)
        return multi ? "partial" : "partial";
    if (lines >= 20 || relevant.length >= 1)
        return "thin";
    return "none";
}
function buildCorpusReport(sourcesDir, locale) {
    ...
    const multi = manifest.entries.filter((e) => e.signals.includes("multi_correct_mcq"));
    ...
    if (multi.length > 0)
        score -= 1;
    if (stubs.length > manifest.entries.length / 3)
        score -= 1;
...
export function diffQualityReports(baseline, current) {
    const baseMap = new Map(baseline.matrix.cells.map((c) => [`${c.row}\0${c.col}`, c]));
    const matrixChanges = [];
...
export function buildQualityReport(options = {}) {
    ...
    if (sourcesDir) {
        ...
        if (!projectRoot) {
            throw new SdmError("NOT_A_PROJECT", `Not a SDM methodology project: ${startDir}. Run: sdm init`);
        }
...
export function formatQualityReportText(document, locale) {
    const loc = locale ?? document.locale;
    const lines = [];
    const title = loc === "ru" ? "Сводный отчёт качества SDM" : "SDM quality report";
    lines.push(`=== ${title} ===`);
    ...
    if (document.diff) {
        ...
        for (const ch of document.diff.matrixChanges.slice(0, 12)) {
            lines.push(`  ${ch.row} × ${ch.col}: ${ch.fromSymbol} → ${ch.toSymbol}`);
        }
    }
    lines.push("");
