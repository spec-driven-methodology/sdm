...
export function kitModuleDisplayTitle(skill) {
...
export function kitMetaDepthWeight(depth, weight, depthBand) {
    const band = depthBand ?? labelDepth(depth).band;
    return `${band} · глубина ${depth} · вес ${weight}`;
}
export function kitChecklistIntroHtml() {
...
export function kitChecklistRowLabel(item) {
    return `${item.depthBand} (${item.depthLabel})`;
}
export function kitWarningView(w, skillTitle) {
    const skill = skillTitle ?? w.skill ?? "навык";
    switch (w.code) {
        case "KIT_NO_PROBE_QUESTION":
            return {
                title: `Нет вопросов для интервьюера: «${skill}»`,
                detail: "Шпаргалка эксперта показывает только открытые и кодовые вопросы с эталоном ответа. Тестовые вопросы с вариантами сюда не попадают — они для тестового экспорта.",
                action: "Добавьте 1–3 открытых вопроса с объяснениями в Библиотеку SDM (как на интервью), затем пересоберите шпаргалку: sdm export kit … --out exports/kit-….html",
            };
        case "KIT_EXPLANATION_MISSING":
            return {
                ...
                action: "Допишите объяснение в Библиотеке SDM (library/questions), затем пересоберите шпаргалку.",
            };
        ...
        case "KIT_NO_WORK_SAMPLE":
            return {
                ...
                detail: "Для уровней middle+ нужна хотя бы одна кодовая проба (type=code) с рубрикой.",
                action: "Добавьте code-вопрос с rubric и evidence через question add, затем пересоберите шпаргалку.",
            };
        case "KIT_PROBE_RUBRIC_MISSING":
            return {
                title: `Нет рубрики у вопроса ${w.questionId ?? ""}`.trim(),
                detail: "Открытые и кодовые пробы должны иметь scored rubric (0–3) для интервьюера.",
        ...
        case "KIT_GLOSSARY_FALLBACK_SKILL":
            return {
                ...
                detail: "Нет library/terms, привязанных к навыкам уровня — показываем fallback из ontology.",
        ...
        case "KIT_SECURITY_SKILL_MISSING":
            return {
                ...
                detail: w.message,
                action: "Добавьте required skill с topics/id про security/privacy/injection.",
            };
...
export function kitEmptyProbesHtml(input) {
    const testNote = input.assessmentQuestionCount > 0
...
export function kitWarningsSummaryHtml(warnings, skillTitles) {
...
//# sourceMappingURL=kit-ui-copy.js.map