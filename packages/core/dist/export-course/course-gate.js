...
function buildGateError(blocking) {
    ...
    const lines = blocking.map((w) => `  [${w.code}]${w.skill ? ` skill="${w.skill}"` : ""}${w.topic ? ` topic="${w.topic}"` : ""} — ${w.message}`);
    const err = new SdmError("COURSE_GATE_BLOCKED", `Course gate (strict) blocked export:\n${lines.join("\n")}\n\nRun \`sdm course heal\` to auto-fix, or set quality.courseGate to "soft" or "off".`);
    err.details = details;
    return err;
}
/**
...
export function assertCourseContextReady(input) {
    ...
    if (input.strictContext && input.depth === "detailed") {
        ...
        if (critical.length > 0) {
            ...
            throw new SdmError("COURSE_CONTEXT_THIN", `TeachingContext too thin for depth=detailed under --strict-context (${codes}). Enrich skill description/topics first.`);
        }
    }
    // courseGate from quality config
    const quality = loadQualityConfig(input.projectRoot);
    const courseGate = quality.courseGate ?? "off";
    if (courseGate !== "strict")
        return;
    const blocking = input.warnings.filter((w) => COURSE_GATE_ERROR_CODES.has(w.code));
    if (blocking.length === 0)
        return;
    throw buildGateError(blocking);
}
//# sourceMappingURL=course-gate.js.map