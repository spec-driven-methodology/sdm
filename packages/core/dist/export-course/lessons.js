...
import { COURSE_OVERVIEW_SKILL_ID, layoutForFormat, } from "./types.js";
/**
 ...
 */
export function buildLessonStubs(skill, questions, format, scopedTopic) {
    const labelFor = (topic) => skill.topic_labels[topic]?.trim() || topic;
    ...
    const topicSet = new Set(skill.topics);
 ...
 */
export function mergeTopicLabels(skills) {
    const labels = {};
 ...
 */
export function seedGlossaryFromTopics(topics, labels) {
    const seen = new Set();
    const entries = [];
 ...
 */
export function orderSkillsByDepends(skillIds, graph) {
    ...
    const subgraph = {
        skills: new Map([...allowed].map((id) => [id, graph.skills.get(id)])),
        dependsOn: new Map([...allowed].map((id) => [
            id,
