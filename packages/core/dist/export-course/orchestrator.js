...
import { buildCoursePackageId, hashContentRevision, buildRevisionByModule, } from "../export-package-identity.js";
...
import { buildLessonStubs, buildOverviewModule, mergeTopicLabels, orderSkillsByDepends, seedGlossaryFromTopics, } from "./lessons.js";
import { collectLearningWarnings, collectProseLocaleWarnings, collectGlossaryMissingTerms, collectDuplicateLessons, } from "./warnings.js";
import { assertCourseContextReady } from "./course-gate.js";
import { COURSE_OVERVIEW_SKILL_ID, EXPORT_COURSE_SCHEMA, layoutForFormat, parseCourseDepth, resolveCourseFormat, } from "./types.js";
 ...
 */
export function exportCourse(options) {
    ...
    const { ordered, cycleFallback } = orderSkillsByDepends(resolved.skillIds, graph);
    ...
    const teachingSkills = skills.map((s) => {
    ...
    const relatedToEdges = [];
    const scoped = new Set(ordered);
    for (const s of skills) {
        for (const d of s.depends_on) {
            if (scoped.has(d))
                dependsOnEdges.push({ from: s.id, to: d });
        }
        for (const r of s.related_to) {
            if (scoped.has(r))
                relatedToEdges.push({ from: s.id, to: r });
        }
    }
    ...
    const warnings = [];
    const glossary = format === "course" && teachingContext.topics.length > 0
        ? seedGlossaryFromTopics(teachingContext.topics, mergeTopicLabels(skills))
        : undefined;
    warnings.push(...collectLearningWarnings({
        skills,
        questionsBySkill,
    ...
    if (deprecatedConcept) {
        warnings.push({
            ...
            message: 'format "concept" is deprecated; use "notes" (Конспект). Normalized to notes.',
        });
    }
    ...
    const skillModules = skills.map((skill) => {
        ...
        if (resolved.forcedQuestionIds && includePractice) {
            const forced = [...resolved.forcedQuestionIds].filter((qid) => qs.some((q) => q.id === qid));
        ...
        return {
            ...
            prerequisites: skill.depends_on.length > 0 ? [...skill.depends_on] : undefined,
        };
    });
    const modules = shouldIncludeOverviewModule(format, resolved.scopeMeta)
    ...
    if (!planOnlyFlag) {
        const allEmpty = modules.every((m) => m.lessons.every((l) => !l.body || l.body.trim() === ""));
        if (allEmpty) {
            throw new SdmError("COURSE_ALL_EMPTY", "Все уроки курса пусты (тела lessons[].body не заполнены).\n" +
