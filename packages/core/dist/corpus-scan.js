...
export const CORPUS_MANIFEST_SCHEMA = "sdm.corpus.manifest/v1";
...
function walkMarkdownFiles(root) {
    ...
    while (stack.length > 0) {
        ...
        for (const name of names) {
            if (name === "node_modules" || name === ".git" || name === ".sdm")
                continue;
...
function classifyArtifact(relPath, content, lineCount) {
    const p = relPath.toLowerCase();
    const base = p.split("/").pop() ?? p;
    const looksLikeBank = /bank|assignment|question|testing-qa|prompt-engineering|analyst|developer/.test(p) ||
        /✅/.test(content) ||
        /single_choice|multi_choice|вариант/.test(content);
    if (/matrix|competenc|паутин|digital-trace|следы/.test(p) ||
    ...
    if (looksLikeBank) {
        const status = /bank\.md|qa-bank/.test(p)
            ? "quarry"
            : "unknown";
...
export function extractTopicCandidates(manifest) {
    ...
    for (const e of manifest.entries) {
        for (const h of e.headings) {
            ...
            if (/prompt/i.test(h))
                topics.add("Prompting");
            if (/context|контекст/i.test(h))
                topics.add("Context management");
            if (/agent/i.test(h))
                topics.add("Agents team");
            if (/tool|инструмент/i.test(h))
                topics.add("Creating tools");
            if (/skill|навык/i.test(h) && !/prompt/i.test(h))
                topics.add("Creating skills");
        }
    }
    return [...topics];
}
export function scanCorpusSources(options) {
    const sourcesRoot = resolve(options.sourcesDir);
    if (!existsSync(sourcesRoot) || !statSync(sourcesRoot).isDirectory()) {
        throw new SdmError("CORPUS_SOURCES_NOT_FOUND", `Sources directory not found or not a directory: ${sourcesRoot}`);
    }
    ...
    for (const full of files) {
        ...
        entries.push(CorpusEntrySchema.parse({
            path: rel,
            artifactType,
            status,
...
//# sourceMappingURL=corpus-scan.js.map