...
const MAX_DEPTH = 4;
const SECRET_KEY = /pass(word)?|token|secret|authorization|api[_-]?key|credential/i;
export function isLoggingEnvDisabled() {
...
export function resolveProjectRootForLog(startDir, explicit) {
    if (explicit)
        return explicit;
...
export function loadLoggingSettings(projectRoot) {
    const defaults = {
        enabled: true,
...
export function actionLogDir(projectRoot) {
    return join(projectRoot, ".sdm", "logs");
}
export function rotateLogFile(filePath, maxFiles) {
    if (maxFiles < 1)
        return;
...
function appendLine(filePath, line, settings) {
    if (existsSync(filePath)) {
...
export function appendActionLog(entry) {
    ...
    if (entry.args !== undefined)
        record.args = redactForLog(entry.args);
    if (entry.code)
        record.code = entry.code;
    if (entry.durationMs !== undefined)
        record.durationMs = entry.durationMs;
    if (entry.summary !== undefined)
        record.summary = redactForLog(entry.summary);
...
export async function withActionLog(options) {
