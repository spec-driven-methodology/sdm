...
export function statusForCount(count, minOk = MIN_OK_QUESTIONS) {
    if (count <= 0)
        return "missing";
    if (count < minOk)
        return "thin";
...
export function countQuestionsBySkill(questions) {
...
export function maxDifficultyBySkill(questions) {
    const maxes = new Map();
    for (const q of questions) {
...
export function depthRatio(achieved, required, questionCount) {
    if (required <= 0) {
        return questionCount > 0 ? 1 : 0;
    }
    return achieved / required;
}
function uncoveredTopicsForSkill(skillTopics, skillQuestions) {
...
function resolveLegacyStatus(questionCount, ratio, minOk) {
    if (questionCount <= 0)
        return "missing";
    if (questionCount < minOk || ratio < DEPTH_OK_RATIO)
        return "thin";
    return "ok";
}
function buildWorkItemsForSkill(skill, skillQuestions, minOk, quality) {
    const items = [];
    const band = skill.missingDifficultyBand ??
        (skill.depth > 0
            ? {
...
export function computeCoverage(level, questions, options) {
    const quality = options?.quality ?? DEFAULT_QUALITY_CONFIG;
    const minOk = options?.minOkQuestions ??
    ...
    const skills = level.requirements.map((req) => {
        ...
        if (status === "missing") {
            ...
            if (questionCount < minOk)
                reasons.push("below_min_questions");
            if (ratio < DEPTH_OK_RATIO)
                reasons.push("depth_ratio");
        }
        const missingDifficultyBand = status !== "ok" && req.depth > 0 && achievedDepth < req.depth * DEPTH_OK_RATIO
            ? {
        ...
        const finalReasons = status === "ok" ? [] : [...new Set(reasons.length ? reasons : [status])];
    ...
    const workItems = quality.coverageMode === "blueprint"
        ? skills
            .filter((s) => s.status !== "ok")
            .flatMap((s) => buildWorkItemsForSkill(s, bySkill.get(s.skill) ?? [], minOk, quality))
        : [];
    return {
