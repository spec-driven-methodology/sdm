 ...
 */
export function resolveExportQuestionFilter(input) {
 ...
 */
export function applyExportQuestionFilter(questions, filter) {
    if (!filter)
        return questions;
    const byId = new Map(questions.map((q) => [q.id, q]));
    const missing = filter.ids.filter((id) => !byId.has(id));
    if (missing.length > 0) {
        throw new SdmError("EXPORT_QUESTION_NOT_FOUND", `Question id(s) not in export candidates after prior filters: ${missing.join(", ")}`);
    }
...
//# sourceMappingURL=export-question-filter.js.map