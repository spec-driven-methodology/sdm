 ...
 */
export function resolveSpecraHome(explicit) {
    if (explicit?.trim()) {
        const root = resolve(explicit.trim());
        if (!isSdmPackageRoot(root)) {
            throw new SdmError("SDM_HOME_NOT_FOUND", `Not a SDM package root (need ABOUT.md + package.json name "sdm"): ${root}`);
        }
        return root;
    }
    ...
    throw new SdmError("SDM_HOME_NOT_FOUND", "Could not locate SDM package root (ABOUT.md). Set SDM_HOME or pass sdmHome.");
}
function parseAboutFrontmatter(text) {
    const match = text.match(/^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n([\s\S]*))?$/);
    if (!match) {
        throw new SdmError("ABOUT_CANON_INVALID", "ABOUT.md must start with YAML frontmatter (--- ... ---)");
    }
    let raw;
    try {
        ...
        throw new SdmError("ABOUT_CANON_INVALID", `ABOUT.md frontmatter is not valid YAML: ${err instanceof Error ? err.message : String(err)}`);
    }
    const parsed = AboutCanonFrontmatterSchema.safeParse(raw);
    if (!parsed.success) {
        throw new SdmError("ABOUT_CANON_INVALID", `ABOUT.md frontmatter missing required fields: ${parsed.error.message}`);
    }
    return parsed.data;
}
function parseSkillPurpose(skillMd, fallbackId) {
...
export function listPortableSkills(agentsRoot) {
 ...
 */
export function getProductVersion(explicitHome) {
    const home = resolveSpecraHome(explicitHome);
    const pkgPath = join(home, "package.json");
    const pkg = JSON.parse(readFileSync(pkgPath, "utf8"));
    if (!pkg.version || typeof pkg.version !== "string") {
        throw new SdmError("ABOUT_VERSION_MISSING", `package.json at ${pkgPath} has no version string`);
    }
    return pkg.version;
}
/**
 ...
 */
export function buildAbout(options = {}) {
...
export function formatAboutText(payload) {
    const lines = [
