...
function topicFilePath(projectRoot, slug) {
    return join(projectRoot, "library", "topics", `${slug}.yaml`);
}
function topicsDir(projectRoot) {
...
export function loadTopics(projectRoot) {
 ...
 */
export function buildTopicRegistry(projectRoot, courseModules) {
    const registered = new Map(loadTopics(projectRoot).map((t) => [t.id, t]));
    const registeredSlugs = new Set(registered.keys());
    const skills = loadAllSkills(projectRoot);
    const { questions } = loadQuestions(projectRoot);
    const allSkillTopics = new Set();
    for (const s of skills) {
        for (const t of s.topics)
            allSkillTopics.add(t);
    }
    const allQuestionTopics = new Set();
    for (const q of questions)
        for (const t of q.topics)
            allQuestionTopics.add(t);
    const allCourseTopics = new Set();
    if (courseModules) {
        for (const mod of courseModules) {
            ...
            for (const lesson of mod.lessons) {
                if (lesson.topic)
                    allCourseTopics.add(lesson.topic);
            }
        }
    }
    const unregisteredFromSkills = [...allSkillTopics].filter((t) => !registeredSlugs.has(t));
    const orphanQuestionTopics = [...allQuestionTopics].filter((t) => !registeredSlugs.has(t) && !allSkillTopics.has(t));
    const unregisteredFromCourses = [...allCourseTopics].filter((t) => !registeredSlugs.has(t) && !allSkillTopics.has(t));
    const used = new Set([...allSkillTopics, ...allQuestionTopics]);
 ...
 */
export function syncTopicRegistryFromSkills(projectRoot) {
...
export function addTopic(projectRoot, input) {
    ...
    if (existsSync(path) && !input.force) {
        throw new SdmError("TOPIC_EXISTS", `Topic "${id}" already exists at ${path}. Use --force.`);
    }
    const topic = {
        id,
 ...
 */
export function mergeRegistryLabelsIntoSkills(projectRoot) {
    const topics = loadTopics(projectRoot);
    const labelMap = new Map();
    for (const t of topics) {
        if (t.label && t.label !== t.id)
            labelMap.set(t.id, t.label);
    }
    if (labelMap.size === 0)
        return 0;
    const skills = loadAllSkills(projectRoot);
