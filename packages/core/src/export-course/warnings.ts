...
export function detectTruncation(body: string): boolean {
  ...
  if (
    truncationMarkers.some((m) => lastWord.endsWith(m)) &&
    lastWord.length < 7
  ) {
    return true;
  }
...
export const TRUNCATION_WARNING_CODE = "LESSON_TRUNCATED" as const;
 ...
 */
export function detectProseLocaleMixed(body: string): boolean {
...
export function collectProseLocaleWarnings(
...
export function isCriticalWarning(w: CourseWarning): boolean {
  return (
    w.code === "SKILL_DESCRIPTION_THIN" || w.code === "SKILL_TOPICS_EMPTY"
  );
}
 ...
 */
export function collectLearningWarnings(
  input: CollectLearningWarningsInput,
): CourseWarning[] {
  const warnings: CourseWarning[] = [];
  ...
  if (input.cycleFallback) {
    warnings.push({
      ...
      message:
        "depends_on cycle in scope; module order fell back to alphabetical skill ids.",
    });
  }
  ...
  for (const skill of input.skills) {
    ...
    const referencedBy = input.skills.some(
      (other) =>
        other.id !== skill.id &&
 ...
 */
export function collectGlossaryMissingTerms(
  modules: CourseModule[],
  ...
  for (const mod of modules) {
    ...
    for (const lesson of mod.lessons) {
      ...
      for (const slug of topicSlugs) {
        if (glossSet.has(slug.toLowerCase())) continue;
 ...
 */
export function collectDuplicateLessons(
  modules: CourseModule[],
  threshold = 0.7,
): CourseWarning[] {
  const warnings: CourseWarning[] = [];
