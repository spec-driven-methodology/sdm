 ...
 */
export function extractTopicsFromModules(
  modules: CourseModule[],
): Map<string, string[]> {
  const bySkill = new Map<string, Set<string>>();
  for (const mod of modules) {
    if (mod.kind === "overview") continue;
    const seen = bySkill.get(mod.skill) ?? new Set<string>();
 ...
 */
export function mergeTopicsIntoSkill(
  declared: string[],
  courseTopics: string[],
): string[] {
 ...
 */
export function suggestDescriptionFromModules(
  modules: CourseModule[],
  skillId: string,
): string | undefined {
  const mod = modules.find((m) => m.kind !== "overview" && m.skill === skillId);
  if (!mod || mod.lessons.length === 0) return undefined;
