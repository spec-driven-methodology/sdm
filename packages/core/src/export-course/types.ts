...
export const EXPORT_COURSE_SCHEMA = "sdm.export.course/v1" as const;

export const COURSE_DEPTHS = ["brief", "standard", "detailed"] as const;
...
export const COURSE_LAYOUTS = ["single_doc", "modular_course"] as const;
...
export interface TeachingContext {
  ...
  graphEdges: {
    dependsOn: Array<{ from: string; to: string }>;
    relatedTo: Array<{ from: string; to: string }>;
  };
}
...
export const COURSE_MODULE_KINDS = ["overview", "skill"] as const;
...
export const COURSE_OVERVIEW_SKILL_ID = "course-overview" as const;
...
export function parseCourseDepth(value: string | undefined): CourseDepth {
  const v = (value ?? "standard").toLowerCase();
  if ((COURSE_DEPTHS as readonly string[]).includes(v)) {
    return v as CourseDepth;
  }
  throw new SdmError(
    "COURSE_DEPTH_INVALID",
    `Invalid course depth "${value}". Use brief, standard, or detailed.`,
  );
}

export function resolveCourseFormat(value: string | undefined): {
  format: CourseFormat;
  ...
  if ((COURSE_FORMATS as readonly string[]).includes(v)) {
    return { format: v as CourseFormat, deprecatedConcept: false };
  }
  throw new SdmError(
    "COURSE_FORMAT_INVALID",
    `Invalid course format "${value}". Use howto, notes, cheatsheet, or course.`,
  );
}

export function parseCourseFormat(value: string | undefined): CourseFormat {
  return resolveCourseFormat(value).format;
}

export function layoutForFormat(format: CourseFormat): CourseLayout {
