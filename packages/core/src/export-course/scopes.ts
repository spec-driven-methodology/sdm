...
export interface ResolvedScope {
  ...
  profile?: string;
  level?: Level;
  requirementsBySkill: Map<string, { depth: number; weight: number }>;
...
export function resolveScope(
  ...
  if (fromGaps && !(profileOpt && levelOpt)) {
    throw new SdmError(
      ...
      "fromGaps requires --profile and --level.",
    );
  }
  if (fromQuestions.length > 0 && (skillOpt || fromGaps)) {
    throw new SdmError(
      "COURSE_SCOPE_INVALID",
      "fromQuestions cannot be combined with --skill or --from-gaps.",
    );
  }
  if (
    ...
    throw new SdmError(
      "COURSE_SCOPE_INVALID",
      "Provide --profile/--level, --from-gaps with profile/level, --skill, --topic, or --from-questions.",
    );
  }
  if ((profileOpt && !levelOpt) || (!profileOpt && levelOpt)) {
    throw new SdmError(
      "COURSE_SCOPE_INVALID",
      "Profile and level must be provided together (unless using --skill / --topic / --from-questions alone).",
    );
  }
  ...
  if (fromQuestions.length > 0) {
    ...
    let skillIds = [...new Set(selected.map((q) => q.skill))].sort((a, b) =>
      a.localeCompare(b),
    );
    if (topicOpt) {
      skillIds = skillIds.filter((sid) => {
        ...
        return (
          skill.topics.includes(topicOpt) ||
      ...
      if (skillIds.length === 0) {
        throw new SdmError(
          "COURSE_SCOPE_EMPTY",
          `No skills/questions match topic "${topicOpt}" for the given question ids.`,
        );
      }
    }
    return {
  ...
  if (skillOpt && !fromGaps && !(profileOpt && levelOpt)) {
    ...
    if (topicOpt && !skill.topics.includes(topicOpt)) {
      const hasQ = allQuestions.some(
        (q) => q.skill === skillOpt && q.topics.includes(topicOpt),
      );
      if (!hasQ) {
        throw new SdmError(
          "COURSE_SCOPE_EMPTY",
          `Skill "${skillOpt}" has no topic "${topicOpt}".`,
        );
      }
    }
    return {
  ...
  if (topicOpt && !(profileOpt && levelOpt) && !skillOpt && !fromGaps) {
    ...
    if (skillIds.length === 0) {
      throw new SdmError(
        "COURSE_SCOPE_EMPTY",
        `No skills or questions found for topic "${topicOpt}".`,
      );
    }
    return {
      skillIds,
  ...
  if (!profileOpt || !levelOpt) {
    throw new SdmError(
      "COURSE_SCOPE_INVALID",
      "Provide --profile/--level, --skill, --topic, or --from-questions.",
    );
  }
  ...
  const requirementsBySkill = new Map(
    level.requirements
      ...
      .map((r) => [r.skill, { depth: r.depth, weight: r.weight }] as const),
  );

  let mode: CourseScopeMeta["mode"] = "profile_level";
  if (fromGaps) mode = "from_gaps";
  else if (topicOpt) mode = "topic_in_level";
  else if (skillOpt) mode = "skill";
...
export function loadSkillIdsWithTopic(
  projectRoot: string,
  topic: string,
): string[] {
...
export function shouldIncludeOverviewModule(
  format: CourseFormat,
  scope: CourseScopeMeta,
): boolean {
  if (format !== "course") return false;
  if (!scope.profile || !scope.level) return false;
  // Explicit single-skill / question packs stay without mandatory overview.
  if (scope.mode === "skill" || scope.mode === "from_questions") return false;
  if (scope.mode === "topic" && !scope.profile) return false;
  return (
    scope.mode === "profile_level" ||
