...
import { RequirementSchema, type Level } from "./schemas.js";
import { readYamlFile } from "./yaml.js";

function normalizeTeamRaw(raw: unknown): unknown {
...
export function teamFilePath(projectRoot: string, teamId: string): string {
  return join(projectRoot, "certifications", "teams", `${teamId}.yaml`);
}

export function loadTeam(projectRoot: string, teamId: string): Team {
  const path = teamFilePath(projectRoot, teamId);
  if (!existsSync(path)) {
    throw new SdmError(
      "TEAM_NOT_FOUND",
      `Team "${teamId}" not found under certifications/teams/`,
    );
  }
 ...
 */
export function resolveLevelForTeam(
  projectRoot: string,
  profileId: string,
  levelId: string,
  teamId?: string,
  ...
  if (team.profile !== profileId) {
    throw new SdmError(
      ...
      `Team "${teamId}" belongs to profile "${team.profile}", not "${profileId}"`,
    );
  }
