...
export function findProjectRoot(startDir: string): string {
  ...
  throw new SdmError(
    ...
    `No SDM methodology project found (missing ${PROJECT_MANIFEST}). Run from a project directory or: sdm init`,
  );
}
 ...
 */
export function locateProject(startDir: string): { root: string; name: string } | null {
  try {
 ...
 */
export function listMethodologyProjects(
  workspaceDir: string,
  maxDepth = 2,
...
function readYamlName(dir: string): string {
  try {
    const yamlPath = join(dir, PROJECT_MANIFEST);
    const raw = readFileSync(yamlPath, "utf8");
    const doc = load(raw) as { name?: string } | undefined;
