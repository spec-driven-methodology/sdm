...
export function weightSumIsValid(sum: number): boolean {
  return Number.isFinite(sum) && Math.abs(sum - 1) <= WEIGHT_SUM_EPSILON;
}

export function assertWeightSum(weights: readonly number[], context?: string): void {
  const sum = sumWeights(weights);
  if (!weightSumIsValid(sum)) {
    const suffix = context ? ` (${context})` : "";
    throw new SdmError(
      "WEIGHT_SUM_INVALID",
      `Requirement weights must sum to 1 within ${WEIGHT_SUM_EPSILON} (got ${sum})${suffix}`,
    );
  }
}
...
export function normalizeWeights(weights: readonly number[]): {
  ...
  if (!Number.isFinite(sum) || sum <= 0) {
    throw new SdmError(
      "WEIGHT_SUM_INVALID",
      "Requirement weights must sum to a positive total to normalize",
    );
  }
  if (weightSumIsValid(sum)) {
...
export function parseWeightTransfer(raw: string): { skill: string; amount: number } {
  const idx = raw.lastIndexOf(":");
  if (idx <= 0 || idx === raw.length - 1) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --from "${raw}". Expected skill:amount`,
    );
  }
  ...
  if (!skill) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --from "${raw}": empty skill id`,
    );
  }
  if (!Number.isFinite(amount) || amount <= 0 || amount > 1) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --from "${raw}": amount must be a number in (0, 1]`,
    );
  }
  return { skill, amount };
}
...
export function parseWeightSet(raw: string): { skill: string; weight: number } {
  const idx = raw.indexOf("=");
  if (idx <= 0 || idx === raw.length - 1) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --set "${raw}". Expected skill=weight`,
    );
  }
  const skill = raw.slice(0, idx).trim();
  ...
  if (!skill) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --set "${raw}": empty skill id`,
    );
  }
  if (!Number.isFinite(weight) || weight < 0 || weight > 1) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --set "${raw}": weight must be a number in 0..1`,
    );
  }
  return { skill, weight };
}
...
export function weightMap(
  requirements: readonly { skill: string; weight: number }[],
): Record<string, number> {
  const map: Record<string, number> = {};
  for (const r of requirements) map[r.skill] = r.weight;
  return map;
}
