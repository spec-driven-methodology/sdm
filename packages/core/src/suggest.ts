...
type ProfileRef = { profile: string; levels: string[] };

function listProfiles(projectRoot: string): ProfileRef[] {
...
function resolveFocus(
  ...
  if (profileId && levelId) {
    ...
    if (level.profile && level.profile !== profileId) {
      throw new SdmError(
        ...
        `Level "${levelId}" belongs to profile "${level.profile}", not "${profileId}"`,
      );
    }
    if (!profile.levels.includes(levelId)) {
      throw new SdmError(
        "PROFILE_LEVEL_MISMATCH",
        `Profile "${profileId}" does not list level "${levelId}"`,
      );
    }
  ...
  if (profileId && !levelId) {
    ...
    throw new SdmError(
      "SUGGEST_FOCUS_REQUIRED",
      `Profile "${profileId}" has multiple levels (${profile.levels.join(", ")}). Pass --level.`,
    );
  }
  ...
  if (!profileId && levelId) {
    throw new SdmError(
      "SUGGEST_FOCUS_REQUIRED",
      "Pass --profile together with --level (or omit both for auto-focus).",
    );
  }
  ...
  throw new SdmError(
    "SUGGEST_FOCUS_REQUIRED",
    `Ambiguous focus (${profiles.length} profile(s)). Pass --profile and --level.`,
  );
}

function exportPathForFocus(
  projectRoot: string,
  profile: string,
  level: string,
): string {
  ...
  for (const entry of readdirSync(exportsDir)) {
    ...
    try {
      const raw = JSON.parse(
        readFileSync(join(exportsDir, entry), "utf8"),
      ) as { schemaVersion?: string; profile?: string; level?: string };
      if (
        raw.schemaVersion === "sdm.export.test/v1" &&
        raw.profile === profile &&
        raw.level === level
      ) {
        return true;
...
function kitExportPathForFocus(
  projectRoot: string,
  profile: string,
  level: string,
): string {
...
function hasKitExportForFocus(
  ...
  for (const entry of readdirSync(exportsDir)) {
    ...
    try {
      ...
      if (
        raw.schemaVersion === "sdm.export.kit/v1" &&
        raw.profile === profile &&
        raw.level === level
      ) {
        return true;
...
function hasPlayer(projectRoot: string): boolean {
  return existsSync(join(projectRoot, "player", "index.html"));
}

function kitLevers(): SuggestLever[] {
  return [
    {
...
function exportLevers(): SuggestLever[] {
  return [
    {
...
function courseLevers(hasGaps: boolean): SuggestLever[] {
  const levers: SuggestLever[] = [
    {
...
function thresholdLevers(): SuggestLever[] {
  return [
    {
      phrase: "строже порог",
...
function volumeLevers(): SuggestLever[] {
  return [
    {
...
function takeTop(items: SuggestItem[], max = 5): SuggestItem[] {
 ...
 */
export function buildSuggest(options: BuildSuggestOptions): SuggestPayload {
  ...
  try {
    ...
    if (err instanceof SdmError && err.code === "PROJECT_ROOT_NOT_FOUND") {
      throw new SdmError(
        "NOT_A_PROJECT",
        `Not a SDM methodology project: ${start}. Run: sdm init`,
      );
    }
    throw err;
  }
  ...
  const nearDups = qualityOn
    ...
    : [];
  const needsQualityHarden =
    workItemCount > 0 || nearDups.length > 0 || (hasGaps && quality.coverageMode === "blueprint");
  ...
  if (hasGaps || needsQualityHarden) {
    suggestions.push({
      ...
      why: `По ${profileId}/${levelId}: не покрыто=${missing}, слабо покрыто=${thin}, покрыто=${okCount}${
...
export function formatSuggestText(payload: SuggestPayload): string {
  const focus =
