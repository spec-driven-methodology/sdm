...
function parseDifficultyRange(
  ...
  if (min > max) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `difficulty min (${min}) must be <= max (${max})`,
    );
  }
  return { min, max };
}

function difficultyForIndex(i: number, count: number, min: number, max: number): number {
  if (count <= 1) return Number(((min + max) / 2).toFixed(2));
  const t = i / (count - 1);
...
function buildStub(
  ...
  const topicHint = extras?.mustCoverTopic
    ...
    : "";
  const stub: QuestionDraftStub = {
    index,
    skill: skillId,
    type,
    difficulty,
    text: `[DRAFT ${index}] Replace with a real question about ${skillName}${
      extras?.mustCoverTopic ? ` (topic: ${extras.mustCoverTopic})` : ""
    }`,
    instructions:
      `Replace text (and options/correct for choice types, or expected for open), then call question validate then question add.${topicHint} Do not write library YAML by hand.`,
...
export function generateQuestions(
  ...
  const typeMix: TypeMixPreset =
    options.mix !== undefined ? parseTypeMixPreset(options.mix) : "single";

  const draftTypes: Question["type"][] =
    options.type !== undefined
      ? Array.from({ length: count }, () => options.type!)
      : assignTypesForMix(typeMix, count);

  const { min, max } = parseDifficultyRange(
  ...
  let gap: GenerateQuestionsContext["gap"];
  let skillWorkItems: CoverageWorkItem[] = [];
  ...
  const drafts: QuestionDraftStub[] = [];
  if (skillWorkItems.length > 0) {
    for (let i = 0; i < count; i += 1) {
      ...
      const difficulty = difficultyForIndex(
        i % Math.max(skillWorkItems.length, 1),
        Math.max(skillWorkItems.length, 2),
        dMin,
        dMax,
      );
      const type =
      ...
      const topic =
        ...
        (gap?.uncoveredTopics?.length
          ...
          : undefined);
      drafts.push(
        buildStub(skillId, skill.name, i + 1, type, difficulty, {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const topic =
        uncovered.length > 0 ? uncovered[i % uncovered.length] : undefined;
      drafts.push(
        buildStub(skillId, skill.name, i + 1, draftTypes[i]!, difficulty, {
          mustCoverTopic: topic,
          avoidNearIds,
          relatedSkillIds,
        }),
  ...
  const briefLines = drafts.map(
    (d) =>
      `- draft ${d.index}: type=${d.type} difficulty=${d.difficulty}` +
  ...
  return {
    ...
    drafts,
    nextStep:
      "Fill each draft, run `sdm question validate --json`, rewrite until ok, then `sdm question add` (or MCP). Re-check with `sdm cert gaps --json`.",
  };
}
