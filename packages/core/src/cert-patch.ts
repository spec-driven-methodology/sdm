...
import { LevelSchema, type Level } from "./schemas.js";
...
function levelPath(projectRoot: string, levelId: string): string {
  return join(projectRoot, "certifications", "levels", `${levelId}.yaml`);
}

function parseLevel(payload: unknown): Level {
  try {
...
function loadLevelFile(projectRoot: string, levelId: string): { level: Level; path: string } {
...
function applyDonorTransfers(
  bySkill: Map<string, Requirement>,
  fromTransfers: string[],
  levelId: string,
): void {
  for (const raw of fromTransfers) {
    ...
    if (!donor) {
      throw new SdmError(
        "WEIGHT_SKILL_NOT_ON_LEVEL",
        `Donor skill "${skill}" is not on level "${levelId}"`,
      );
    }
    if (donor.weight + 1e-12 < amount) {
      throw new SdmError(
        "WEIGHT_DONOR_INSUFFICIENT",
        `Donor "${skill}" has weight ${donor.weight}, cannot give ${amount}`,
      );
    }
 ...
 */
export function patchCertification(
  projectRoot: string,
  input: PatchCertificationInput,
): PatchCertificationResult {
  ...
  if (
    ...
    throw new SdmError(
      "VALIDATION_FAILED",
      "Provide at least one of --add-requirement, --set-requirement, --remove-requirement, or metadata flags",
    );
  }
  ...
  if (input.profile?.trim()) {
    ...
    if (existing.profile && existing.profile !== profileId) {
      throw new SdmError(
        ...
        `Level "${levelId}" belongs to profile "${existing.profile}", not "${profileId}"`,
      );
    }
  }

  const mutatesRequirements =
    addTriples.length > 0 || setTriples.length > 0 || removeSkills.length > 0;
  ...
  for (const skillId of removeSkills) {
    ...
    if (!prev) {
      throw new SdmError(
        ...
        `Requirement skill "${skillId}" is not on level "${levelId}"`,
      );
    }
  ...
  if (removeSkills.length > 0) {
    if (!absorbInto) {
      throw new SdmError(
        "WEIGHT_TRANSFER_REQUIRED",
        `Removing requirements requires --absorb-into <skill> to keep weight sum = 1 (removed weight ${removedWeight})`,
      );
    }
    if (removeSkills.includes(absorbInto)) {
      throw new SdmError(
        "VALIDATION_FAILED",
        `Cannot --absorb-into "${absorbInto}" which is being removed`,
      );
    }
    ...
    if (!sink) {
      throw new SdmError(
        "WEIGHT_SKILL_NOT_ON_LEVEL",
        `Absorb target "${absorbInto}" is not on level "${levelId}"`,
      );
    }
  ...
  if (addTriples.length > 0) {
    if (fromTransfers.length === 0) {
      throw new SdmError(
        "WEIGHT_TRANSFER_REQUIRED",
        "Adding a requirement with weight requires --from <skill:amount> (no silent renormalize)",
      );
    }
    ...
    throw new SdmError(
      "VALIDATION_FAILED",
      "--from is only valid together with --add-requirement",
    );
  }
  ...
  for (const raw of addTriples) {
    ...
    if (bySkill.has(req.skill)) {
      throw new SdmError(
        "REQUIREMENT_EXISTS",
        `Requirement skill "${req.skill}" already on level "${levelId}". Use --set-requirement to update.`,
      );
    }
    bySkill.set(req.skill, req);
  ...
  if (requirements.length === 0) {
    throw new SdmError(
      "VALIDATION_FAILED",
      "Level must keep at least one requirement after patch",
    );
  }
  ...
  if (mutatesRequirements) {
    ...
    if (!weightSumIsValid(sum)) {
      if (addTriples.length > 0) {
        throw new SdmError(
          "WEIGHT_TRANSFER_REQUIRED",
          `After add/transfer, weights sum to ${sum}; --from amounts must balance new requirement weights to 1`,
        );
      }
