...
export interface CreateCertificationResult {
  profile: Profile;
  level: Level;
  paths: { profile: string; level: string };
  action: "create";
  /** True when input weights were proportionally normalized before persist. */
  weightsNormalized: boolean;
}

export function parseRequirementTriple(raw: string): Requirement {
  const parts = raw.split(":");
  if (parts.length !== 3) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --requirement "${raw}". Expected skill:depth:weight`,
    );
  }
  ...
  if (!skill?.trim()) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --requirement "${raw}": empty skill id`,
    );
  }
  ...
  if (!Number.isFinite(depth) || depth < 0 || depth > 1) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --requirement "${raw}": depth must be a number in 0..1`,
    );
  }
  if (!Number.isFinite(weight) || weight < 0 || weight > 1) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Invalid --requirement "${raw}": weight must be a number in 0..1`,
    );
  }
...
export function createCertification(
  ...
  if (input.requirementTriples.length === 0) {
    throw new SdmError(
      "VALIDATION_FAILED",
      "At least one --requirement skill:depth:weight is required",
    );
  }
    ...
    if (missingSkills.length > 0) {
      ...
      throw new SdmError(
        "SKILL_NOT_FOUND",
        `Skills not found: [${missingSkills.join(", ")}]. Available skills: [${available.slice(0, 10).join(", ")}${available.length > 10 ? ", ..." : ""}]`,
      );
    }
    ...
    const normalizedDepths = parsed.map(req => {
      const d = req.depth;
      if (typeof d === 'string') {
        const depthStr = d as string;
        const key = depthStr.toLowerCase() as keyof typeof depthMap;
    ...
    if (rawSum === 0) {
      throw new SdmError(
        "WEIGHT_SUM_INVALID",
        "Requirement weights sum to zero — at least one non-zero weight is required",
      );
    }
    let normalizedWeights: { weights: number[]; normalized: boolean };
  ...
  try {
    ...
    if (err instanceof SdmError && err.code === "PROFILE_NOT_FOUND") {
      throw new SdmError(
        "PROFILE_NOT_FOUND",
        `Profile "${profileId}" not found. Run profile create first.`,
      );
    }
    throw err;
  }
  ...
  if (existsSync(lPath) && !input.force) {
    throw new SdmError(
      "LEVEL_EXISTS",
      `Level file already exists: ${lPath}. Use --force to overwrite.`,
    );
  }
