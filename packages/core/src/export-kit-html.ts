import type { ExportKitDocument, KitModule, KitProbe } from "./export-kit.js";
import {
  kitChecklistIntroHtml,
  kitEmptyProbesHtml,
  kitMetaDepthWeight,
  kitWarningsSummaryHtml,
} from "./kit-ui-copy.js";
...
function renderProbe(probe: KitProbe): string {
  const parts = [
    `<div class="qbox">`,
  ...
  if (probe.evidence) {
    parts.push(
      `<p class="q-meta"><strong>Evidence:</strong> ${escapeHtml(probe.evidence)}</p>`,
    );
  }
  if (probe.minDepthBand) {
    parts.push(
      `<p class="q-meta"><strong>Закрывает:</strong> ${escapeHtml(probe.minDepthBand)} (${escapeHtml(probe.minDepthLabel ?? "")})</p>`,
    );
  }
  if (probe.explanation) {
    parts.push(
      `<p class="q-expected"><strong>Эталон:</strong> ${escapeHtml(probe.explanation)}</p>`,
    );
  }
  ...
  if (probe.rubric && probe.rubric.length > 0) {
    ...
    for (const row of probe.rubric) {
      parts.push(
        `<li><strong>${row.score}:</strong> ${escapeHtml(row.description)}</li>`,
      );
    }
    parts.push("</ul>");
  }
...
function renderModule(mod: KitModule, index: number): string {
  const topics =
    mod.topics.length > 0
...
export function renderKitHtml(doc: ExportKitDocument): string {
  const tocItems = doc.modules
    .map(
      (m, i) =>
        `<li><a href="#m${i}">${escapeHtml(m.title)}</a></li>`,
    )
    .join("\n");
  ...
  const checklistRows = doc.checklist
    .map(
      (c) =>
        `<tr><td>${escapeHtml(c.skillName)}</td><td>${escapeHtml(c.depthBand)} <span class="muted">(${escapeHtml(c.depthLabel)})</span></td><td>${c.depth}</td><td>${c.weight}</td></tr>`,
    )
    .join("\n");
  ...
  const glossaryRows = doc.glossary
    .map(
      (g) =>
        `<tr><td><code>${escapeHtml(g.id)}</code></td><td>${escapeHtml(g.term)}</td><td>${escapeHtml(g.definition)}</td></tr>`,
    )
    .join("\n");
  ...
  const skillTitles = new Map(
    doc.modules.map((m) => [m.skill, m.title] as const),
  );
