 ...
 */
export function syncPlayerAssets(options: SyncPlayerOptions): SyncPlayerResult {
  ...
  if (!existsSync(playerSrc)) {
    throw new SdmError(
      "PLAYER_TEMPLATE_MISSING",
      `Player template not found at ${playerSrc}`,
    );
  }
...
export function copyPlayerTemplateInto(
  targetDir: string,
  created: string[],
  skipped: string[],
  force: boolean,
