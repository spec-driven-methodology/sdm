...
export function healCourseWarnings(input: {
  ...
  for (const mod of skillModules) {
    ...
    for (const t of lessonTopics) {
      if (!existingSet.has(t)) { changed = true; break; }
    }
    if (!changed) continue;

    const merged = [...new Set([...skill.topics, ...lessonTopics])].sort((a, b) =>
      a.localeCompare(b),
    );
  ...
  const thinWarnings = input.warnings.filter(
    (w) => w.code === "SKILL_DESCRIPTION_THIN" && w.skill,
  );
