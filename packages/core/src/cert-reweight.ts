...
function applyTransfer(
  ...
  delta: number,
  fromRaws: string[],
): { requirements: Level["requirements"]; transfers: ReweightCertificationResult["transfers"] } {
  ...
  if (!Number.isFinite(delta) || delta <= 0 || delta > 1) {
    throw new SdmError(
      "VALIDATION_FAILED",
      "--delta must be a number in (0, 1]",
    );
  }
  if (!fromRaws.length) {
    throw new SdmError(
      "VALIDATION_FAILED",
      "Transfer mode requires at least one --from <skill>[:amount]",
    );
  }
  ...
  if (!bySkill.has(target)) {
    throw new SdmError(
      ...
      `Skill "${target}" is not on level "${level.level}"`,
    );
  }

  const donors: Array<{ skill: string; amount: number }> = [];
  if (fromRaws.length === 1 && !fromRaws[0]!.includes(":")) {
    ...
    for (const raw of fromRaws) {
      if (!raw.includes(":")) {
        throw new SdmError(
          "VALIDATION_FAILED",
          `Invalid --from "${raw}". With multiple donors use skill:amount`,
        );
      }
  ...
  if (Math.abs(donorTotal - delta) > 1e-9) {
    throw new SdmError(
      "VALIDATION_FAILED",
      `Sum of --from amounts (${donorTotal}) must equal --delta (${delta})`,
    );
  }

  const transfers: ReweightCertificationResult["transfers"] = [];
  for (const donor of donors) {
    const req = bySkill.get(donor.skill);
    if (!req) {
      throw new SdmError(
        "WEIGHT_SKILL_NOT_ON_LEVEL",
        `Donor skill "${donor.skill}" is not on level "${level.level}"`,
      );
    }
    if (donor.skill === target) {
      throw new SdmError(
        "VALIDATION_FAILED",
        "Cannot transfer weight from a skill to itself",
      );
    }
    if (req.weight + 1e-12 < donor.amount) {
      throw new SdmError(
        "WEIGHT_DONOR_INSUFFICIENT",
        `Donor "${donor.skill}" has weight ${req.weight}, cannot give ${donor.amount}`,
      );
    }
...
function applySetMap(
  ...
  for (const skill of levelSkills) {
    if (!setSkills.has(skill)) {
      throw new SdmError(
        "WEIGHT_SUM_INVALID",
        `Incomplete --set map: missing skill "${skill}"`,
      );
    }
  }
  for (const skill of setSkills) {
    if (!levelSkills.has(skill)) {
      throw new SdmError(
        "WEIGHT_SKILL_NOT_ON_LEVEL",
        `Skill "${skill}" is not on level "${level.level}"`,
      );
    }
  }
 ...
 */
export function reweightCertification(
  projectRoot: string,
  input: ReweightCertificationInput,
): ReweightCertificationResult {
  ...
  if (hasTransfer === hasSet) {
    throw new SdmError(
      "VALIDATION_FAILED",
      "Provide either transfer (--skill --delta --from) or full --set map, not both/neither",
    );
  }
  ...
  let transfers: ReweightCertificationResult["transfers"] = [];
