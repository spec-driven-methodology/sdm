 ...
 */
export function buildStudioCoverageView(
  options: BuildStudioCoverageViewOptions,
...
export interface PushStudioCoverageResult {
  ...
  profile: string;
  level: string;
  summary: { missing: number; thin: number; ok: number };
}

/** Build coverage view and write to .sdm/studio/current-view.json */
export function pushStudioCoverage(
  options: BuildStudioCoverageViewOptions,
): PushStudioCoverageResult {
