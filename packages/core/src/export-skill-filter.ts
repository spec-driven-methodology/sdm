 ...
 */
export function resolveExportSkillFilter(
  input: ResolveExportSkillFilterInput,
  levelRequirements: readonly { skill: string }[],
  ...
  if (includeRaw.length > 0 && excludeRaw.length > 0) {
    throw new SdmError(
      "EXPORT_SKILL_FILTER_CONFLICT",
      "Pass either includeSkills / --include-skill or excludeSkills / --exclude-skill, not both.",
    );
  }
  ...
  for (const skill of requested) {
    if (!onLevel.has(skill)) {
      throw new SdmError(
        "EXPORT_SKILL_UNKNOWN",
        `Skill "${skill}" is not on the effective level requirements.`,
      );
    }
  }
 ...
 */
export function applySkillFilterToRequirements(
  requirements: Level["requirements"],
  filter: ExportSkillFilter | undefined,
): ApplySkillFilterToRequirementsResult {
  if (!filter) {
  ...
  if (kept.length === 0) {
    throw new SdmError(
      "EXPORT_FILTER_EMPTY",
      "Skill filter removed all requirements from the export package.",
    );
  }
