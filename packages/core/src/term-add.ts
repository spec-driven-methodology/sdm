...
import { TermSchema, type Term, type TermKind } from "./schemas.js";
...
function termPath(projectRoot: string, id: string): string {
  return join(projectRoot, "library", "terms", `${id}.yaml`);
}

function parseTermOrThrow(payload: unknown): Term {
  try {
...
export function addTerm(
  ...
  if (existsSync(path) && !input.force) {
    throw new SdmError(
      "TERM_EXISTS",
      `Term "${id}" already exists at ${path}. Use --force to overwrite.`,
    );
  }
  ...
  const dup = existing.find(
    (t) => t.id !== id && t.term.toLowerCase() === termLabel.toLowerCase(),
  );
  if (dup && !input.force) {
    throw new SdmError(
      "TERM_DUPLICATE_LABEL",
      `Term label "${termLabel}" already used by "${dup.id}"`,
    );
  }
  ...
  const doc: Term = parseTermOrThrow({
    ...docBody,
    meta: { basis },
  });
