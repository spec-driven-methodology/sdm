...
export function normalizeExpectedInput(expected: string | string[]): string | string[] {
...
export function assertChoiceShapeOrThrow(input: AddQuestionInput): void {
  if (input.type === "single_choice" || input.type === "multi_choice") {
    if (!input.options || input.options.length < 2) {
      throw new SdmError(
        "VALIDATION_FAILED",
        `Type ${input.type} requires at least two --option values`,
      );
    }
    if (input.correct === undefined) {
      throw new SdmError(
        "VALIDATION_FAILED",
        `Type ${input.type} requires --correct (1-based option index)`,
      );
    }
    ...
    for (const c of corrects) {
      if (!Number.isInteger(c) || c < 1 || c > input.options.length) {
        throw new SdmError(
          "VALIDATION_FAILED",
          `--correct ${c} is out of range for ${input.options.length} options (1-based)`,
        );
      }
    }
    if (input.type === "single_choice" && corrects.length !== 1) {
      throw new SdmError(
        "VALIDATION_FAILED",
        "single_choice requires exactly one --correct",
      );
    }
  }
}

export function assertExpectedShapeOrThrow(input: AddQuestionInput): void {
  if (input.expected === undefined) {
    return;
  }
  if (input.type !== "open") {
    throw new SdmError(
      "VALIDATION_FAILED",
      `--expected is only allowed for type open (got ${input.type})`,
    );
  }
}
