...
description: >-
  After writes/generate or when the human asks «what's next?», call SDM
  suggest for next actions + levers (harden quality, stale content,
...
- Human asks «what's next?», «what else can be done?», «what do you suggest?» without a concrete op
...
2. Present **1–3** top `suggestions` as human actions (labels + why). Include a few `levers` (phrases) when relevant:
   - Quality / gaps: «summary quality report», «corpus sources report», «check the draft», «close topics», «enable strict writeGate», «generate drafts», «5 per skill»
   - Export test: «without text answers», «stricter threshold»
   - Export learning: «short howto», «topic notes», «cheat sheet for learner», «course / module with lessons», «only by coverage gaps»
   - Export kit: «cheat sheet for expert», «HTML for interviewer», «like the colleagues reference»
...
4. On human choice → hand off:
   ...
   - `player-sync` / `try-player` → `sdm player sync --force` then open `player/index.html` (tabs Tests|Courses|Cheat sheets)
...
- Mixing **threshold** (pass bar) with **depth/weight** or «more questions»
...
- Leaving `format`/`depth` ambiguous when the human asked for notes vs course vs howto
