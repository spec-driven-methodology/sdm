...
| Skill | When to use |
|-------|-------------|
| [explain-sdm](./explain-sdm/SKILL.md) | "What is SDM?" → MCP `about` (canonical methodology-as-specs; specs·framework·agent; not LMS / not agent harness; competency owners) |
| [guide-suggest](./guide-suggest/SKILL.md) | "What's next?" → `suggest` (harden/validate, stale, gaps, export, player, levers) |
...
| [connect-mcp](./connect-mcp/SKILL.md) | Wire MCP + skills via `mcp install --hosts` (skills by default; Cursor, MultiTool, GigaCode, …) |
...
- Primary wire: `sdm mcp install --hosts cursor,multitool,gigacode` (portable skills included; `--no-skills` / standalone `agent install` for edge cases)
