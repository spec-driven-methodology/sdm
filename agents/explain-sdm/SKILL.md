...
description: >-
  Answer «что такое SDM / что умеет / чем не является» from the canonical
...
- «что такое SDM?» / «what is SDM?»
- «что умеет?» / capabilities / feature list
- «зачем?» / how to position / vs testing platforms / LMS
...
| Level | Formula |
|-------|---------|
| **What** | SDM — фреймворк methodology-as-specs: онтология навыков → библиотека контента → профили/пороги → аудит покрытия и экспорт. Один скелет (**эталон** / единый источник правды) для оценки и обучения |
...
2. Paraphrase for the human using `positioning.what`, `positioning.whatNot`, `positioning.model`, `version`, and `nextSteps`. Prefer the two-level table above when summarizing; mention эталон and harness boundary when relevant.
...
- Describing SDM as «платформа тестирования сотрудников» or «LMS»
- Framing SDM as a full agent harness / multi-model orchestrator
- Narrowing audience to only «методолог» when the human asks who it is for — use competency owners
